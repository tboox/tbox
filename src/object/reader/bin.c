...
static tb_object_t* tb_object_bin_reader_done(tb_gstream_t* stream)
{
	// read bin header
	tb_byte_t data[32] = {0};
...
static tb_size_t tb_object_bin_reader_probe(tb_gstream_t* stream)
{
	// check
	...
	// need it
	tb_byte_t* p = tb_null;
	if (!tb_gstream_need(stream, &p, 3)) return 0;
