...
static tb_object_t* tb_object_bin_reader_func_array(tb_object_bin_reader_t* reader, tb_size_t type, tb_uint64_t size)
{
    ...
    // empty?
    if (!size) return tb_array_init(TB_OBJECT_BIN_READER_ARRAY_GROW, tb_false);

    // init array
    tb_object_t* array = tb_array_init(TB_OBJECT_BIN_READER_ARRAY_GROW, tb_false);
...
static tb_object_t* tb_object_bin_reader_func_number(tb_object_bin_reader_t* reader, tb_size_t type, tb_uint64_t size)
{
    ...
    case TB_NUMBER_TYPE_UINT64:
        number = tb_number_init_from_uint64(tb_basic_stream_bread_u64_be(reader->stream));
        break;
    case TB_NUMBER_TYPE_SINT64:
        number = tb_number_init_from_sint64(tb_basic_stream_bread_s64_be(reader->stream));
        break;
    case TB_NUMBER_TYPE_UINT32:
        number = tb_number_init_from_uint32(tb_basic_stream_bread_u32_be(reader->stream));
        break;
    case TB_NUMBER_TYPE_SINT32:
        number = tb_number_init_from_sint32(tb_basic_stream_bread_s32_be(reader->stream));
        break;
    case TB_NUMBER_TYPE_UINT16:
        number = tb_number_init_from_uint16(tb_basic_stream_bread_u16_be(reader->stream));
        break;
    case TB_NUMBER_TYPE_SINT16:
        number = tb_number_init_from_sint16(tb_basic_stream_bread_s16_be(reader->stream));
        break;
    case TB_NUMBER_TYPE_UINT8:
        number = tb_number_init_from_uint8(tb_basic_stream_bread_u8(reader->stream));
        break;
    case TB_NUMBER_TYPE_SINT8:
        number = tb_number_init_from_sint8(tb_basic_stream_bread_s8(reader->stream));
        break;
