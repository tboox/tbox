 ...
 * implementation
 */
static tb_bool_t tb_object_xplist_writer_func_date(tb_object_xplist_writer_t* writer, tb_object_t* object, tb_size_t level)
{
	// check
	...
	if (time > 0)
	{
		...
		if (tb_gstream_printf(writer->stream, "<date>") < 0) return tb_false;

		// writ date
		tb_tm_t date = {0};
		if (tb_localtime(time, &date))
		{
			if (tb_gstream_printf(writer->stream, 	"%04ld-%02ld-%02ldT%02ld:%02ld:%02ldZ"
								, 	date.year
								, 	date.month
								, 	date.mday
								, 	date.hour
...
static tb_bool_t tb_object_xplist_writer_func_data(tb_object_xplist_writer_t* writer, tb_object_t* object, tb_size_t level)
{
	// check
	tb_assert_and_check_return_val(writer && writer->stream, tb_false);
	...
	if (tb_data_size(object))
	{
		// writ beg
		if (!tb_object_writer_tab(writer->stream, writer->deflate, level)) return tb_false;
		if (tb_gstream_printf(writer->stream, "<data>") < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		...
		for (; p < e && *p; p++, n++)
		{
			...
			if (tb_gstream_printf(writer->stream, "%c", *p) < 0) return tb_false;
		}
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
...
static tb_bool_t tb_object_xplist_writer_func_array(tb_object_xplist_writer_t* writer, tb_object_t* object, tb_size_t level)
{
	// check
	tb_assert_and_check_return_val(writer && writer->stream, tb_false);
	...
	if (tb_array_size(object))
	{
		// writ beg
		if (!tb_object_writer_tab(writer->stream, writer->deflate, level)) return tb_false;
		if (tb_gstream_printf(writer->stream, "<array>") < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
...
static tb_bool_t tb_object_xplist_writer_func_string(tb_object_xplist_writer_t* writer, tb_object_t* object, tb_size_t level)
{
	// check
	tb_assert_and_check_return_val(writer && writer->stream, tb_false);
	...
	{
		if (tb_gstream_printf(writer->stream, "<string>%s</string>", tb_string_cstr(object)) < 0) return tb_false;
	}
	else if (tb_gstream_printf(writer->stream, "<string/>") < 0) return tb_false;
	if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;

	// ok
	return tb_true;
}
static tb_bool_t tb_object_xplist_writer_func_number(tb_object_xplist_writer_t* writer, tb_object_t* object, tb_size_t level)
{
	// check
	tb_assert_and_check_return_val(writer && writer->stream, tb_false);
	...
	case TB_NUMBER_TYPE_UINT64:
		if (!tb_object_writer_tab(writer->stream, writer->deflate, level)) return tb_false;
		if (tb_gstream_printf(writer->stream, "<integer>%llu</integer>", tb_number_uint64(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_SINT64:
		if (!tb_object_writer_tab(writer->stream, writer->deflate, level)) return tb_false;
		if (tb_gstream_printf(writer->stream, "<integer>%lld</integer>", tb_number_sint64(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_UINT32:
		if (!tb_object_writer_tab(writer->stream, writer->deflate, level)) return tb_false;
		if (tb_gstream_printf(writer->stream, "<integer>%u</integer>", tb_number_uint32(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_SINT32:
		if (!tb_object_writer_tab(writer->stream, writer->deflate, level)) return tb_false;
		if (tb_gstream_printf(writer->stream, "<integer>%d</integer>", tb_number_sint32(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_UINT16:
		if (!tb_object_writer_tab(writer->stream, writer->deflate, level)) return tb_false;
		if (tb_gstream_printf(writer->stream, "<integer>%u</integer>", tb_number_uint16(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_SINT16:
		if (!tb_object_writer_tab(writer->stream, writer->deflate, level)) return tb_false;
		if (tb_gstream_printf(writer->stream, "<integer>%d</integer>", tb_number_sint16(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_UINT8:
		if (!tb_object_writer_tab(writer->stream, writer->deflate, level)) return tb_false;
		if (tb_gstream_printf(writer->stream, "<integer>%u</integer>", tb_number_uint8(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_SINT8:
		if (!tb_object_writer_tab(writer->stream, writer->deflate, level)) return tb_false;
		if (tb_gstream_printf(writer->stream, "<integer>%d</integer>", tb_number_sint8(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
#ifdef TB_CONFIG_TYPE_FLOAT
	case TB_NUMBER_TYPE_FLOAT:
		if (!tb_object_writer_tab(writer->stream, writer->deflate, level)) return tb_false;
		if (tb_gstream_printf(writer->stream, "<real>%f</real>", tb_number_float(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_DOUBLE:
		if (!tb_object_writer_tab(writer->stream, writer->deflate, level)) return tb_false;
		if (tb_gstream_printf(writer->stream, "<real>%lf</real>", tb_number_double(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
#endif
	default:
...
static tb_bool_t tb_object_xplist_writer_func_boolean(tb_object_xplist_writer_t* writer, tb_object_t* object, tb_size_t level)
{
	// check
	tb_assert_and_check_return_val(writer && writer->stream, tb_false);
	...
	// writ
	if (!tb_object_writer_tab(writer->stream, writer->deflate, level)) return tb_false;
	if (tb_gstream_printf(writer->stream, "<%s/>", tb_boolean_bool(object)? "true" : "false") < 0) return tb_false;
	if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;

	// ok
	return tb_true;
}
static tb_bool_t tb_object_xplist_writer_func_dictionary(tb_object_xplist_writer_t* writer, tb_object_t* object, tb_size_t level)
{
	// check
	tb_assert_and_check_return_val(writer && writer->stream, tb_false);
	...
	if (tb_dictionary_size(object))
	{
		// writ beg
		if (!tb_object_writer_tab(writer->stream, writer->deflate, level)) return tb_false;
		if (tb_gstream_printf(writer->stream, "<dict>") < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		...
		tb_for_all (tb_dictionary_item_t*, item, tb_dictionary_itor(object))
		{
			...
			if (item && item->key && item->val)
			{
				...
				if (tb_gstream_printf(writer->stream, "<key>%s</key>", item->key) < 0) return tb_false;
				if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
...
static tb_long_t tb_object_xplist_writer_done(tb_gstream_t* stream, tb_object_t* object, tb_bool_t deflate)
{
	// check
	...
	// writ xplist header
	if (tb_gstream_printf(stream, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>") < 0) return -1;
	if (!tb_object_writer_newline(stream, deflate)) return -1;
	if (tb_gstream_printf(stream, "<!DOCTYPE plist PUBLIC \"-//Apple//DTD PLIST 1.0//EN\" \"http://www.apple.com/DTDs/PropertyList-1.0.dtd\">") < 0) return -1;
	if (!tb_object_writer_newline(stream, deflate)) return -1;
	if (tb_gstream_printf(stream, "<plist version=\"1.0\">") < 0) return -1;
	if (!tb_object_writer_newline(stream, deflate)) return -1;
	...
	// writ xplist end
	if (tb_gstream_printf(stream, "</plist>") < 0) return -1;
	if (!tb_object_writer_newline(stream, deflate)) return -1;

	// sync
	if (!tb_gstream_sync(stream, tb_true)) return -1;
