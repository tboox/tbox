...
static tb_bool_t tb_object_json_writer_func_string(tb_object_json_writer_t* writer, tb_object_t* object, tb_size_t level)
{
	...
	{
		if (tb_gstream_printf(writer->stream, "\"%s\"", tb_string_cstr(object)) < 0) return tb_false;
	}
	else if (tb_gstream_printf(writer->stream, "\"\"") < 0) return tb_false;
	if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
...
static tb_bool_t tb_object_json_writer_func_number(tb_object_json_writer_t* writer, tb_object_t* object, tb_size_t level)
{
	...
	case TB_NUMBER_TYPE_UINT64:
		if (tb_gstream_printf(writer->stream, "%llu", tb_number_uint64(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_SINT64:
		if (tb_gstream_printf(writer->stream, "%lld", tb_number_sint64(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_UINT32:
		if (tb_gstream_printf(writer->stream, "%u", tb_number_uint32(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_SINT32:
		if (tb_gstream_printf(writer->stream, "%d", tb_number_sint32(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_UINT16:
		if (tb_gstream_printf(writer->stream, "%u", tb_number_uint16(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_SINT16:
		if (tb_gstream_printf(writer->stream, "%d", tb_number_sint16(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_UINT8:
		if (tb_gstream_printf(writer->stream, "%u", tb_number_uint8(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_SINT8:
		if (tb_gstream_printf(writer->stream, "%d", tb_number_sint8(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
#ifdef TB_CONFIG_TYPE_FLOAT
	case TB_NUMBER_TYPE_FLOAT:
		if (tb_gstream_printf(writer->stream, "%f", tb_number_float(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
	case TB_NUMBER_TYPE_DOUBLE:
		if (tb_gstream_printf(writer->stream, "%lf", tb_number_double(object)) < 0) return tb_false;
		if (!tb_object_writer_newline(writer->stream, writer->deflate)) return tb_false;
		break;
#endif
	default:
	...
	if (tb_dictionary_size(object))
	{
		...
		tb_for_all (tb_dictionary_item_t*, item, tb_dictionary_itor(object))
		{
			...
			if (item && item->key && item->val)
			{
				...
				// writ key
				if (tb_gstream_printf(writer->stream, "\"%s\":", item->key) < 0) return tb_false;

				// writ spaces
				if (!writer->deflate) if (tb_gstream_printf(writer->stream, " ") < 0) return tb_false;
...
static tb_long_t tb_object_json_writer_done(tb_gstream_t* stream, tb_object_t* object, tb_bool_t deflate)
{
	// check
