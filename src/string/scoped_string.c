...
tb_char_t const* tb_scoped_string_rtrim(tb_scoped_string_t* string)
{
    ...
    // init
    tb_char_t*      s = (tb_char_t*)tb_scoped_string_cstr(string);
    ...
    // strip it
    if (e >= s) tb_scoped_string_strip(string, e - s + 1);
    // clear it 
    else tb_scoped_string_clear(string);

    // ok?
    return tb_scoped_string_cstr(string);
}
