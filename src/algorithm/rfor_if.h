 ...
 * }
 *
 * tb_rfor_if(tb_hash_item_t*, item, tb_iterator_head(iterator), tb_iterator_tail(iterator), iterator, item != tb_object_null)
 * {
 ...
 * }
 *
 * tb_rfor_all(tb_hash_item_t*, item, iterator, item != tb_object_null)
 * {
 *      tb_trace_d("item: %p => %p", item->name, item->data);
 * }
 * @endcode
