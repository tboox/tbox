 ...
 * implemention
 */
static tb_bool_t tb_aiop_reactor_epoll_addo(tb_aiop_reactor_t* reactor, tb_handle_t handle, tb_size_t etype)
{
...
static tb_bool_t tb_aiop_reactor_epoll_seto(tb_aiop_reactor_t* reactor, tb_handle_t handle, tb_size_t etype)
{
	tb_aiop_reactor_epoll_t* rtor = (tb_aiop_reactor_epoll_t*)reactor;
	tb_assert_and_check_return_val(rtor && rtor->epfd >= 0, TB_FALSE);
...
static tb_long_t tb_aiop_reactor_epoll_wait(tb_aiop_reactor_t* reactor, tb_long_t timeout)
{	
	tb_aiop_reactor_epoll_t* rtor = (tb_aiop_reactor_epoll_t*)reactor;
...
static tb_void_t tb_aiop_reactor_epoll_sync(tb_aiop_reactor_t* reactor, tb_size_t evtn)
{	
	tb_aiop_reactor_epoll_t* rtor = (tb_aiop_reactor_epoll_t*)reactor;
	tb_assert_and_check_return(rtor && rtor->epfd >= 0 && reactor->aiop && reactor->aiop->objs);
	...
	for (i = 0; i < evtn; i++)
	{
		...
		tb_aioo_t* 		o = reactor->aiop->objs + i;
 ...
 * interfaces
 */
tb_aiop_reactor_t* tb_aiop_reactor_epoll_init(tb_aiop_t* aiop)
{
	// check
	...
	// alloc reactor
	tb_aiop_reactor_epoll_t* rtor = tb_calloc(1, sizeof(tb_aiop_reactor_epoll_t));
	...
	rtor->base.sync = tb_aiop_reactor_epoll_sync;
