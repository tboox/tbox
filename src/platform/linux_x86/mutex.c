...
tb_handle_t tb_mutex_create(tb_char_t const* name)
{
	...
	if (!pmutex || 0 != pthread_mutex_init(pmutex, NULL)) return TB_INVALID_HANDLE;
	else return ((tb_handle_t)pmutex);
}
void tb_mutex_destroy(tb_handle_t hmutex)
{
	TB_ASSERT(hmutex != TB_INVALID_HANDLE);
	if (hmutex == TB_INVALID_HANDLE) return ;
...
{
	TB_ASSERT(hmutex != TB_INVALID_HANDLE);
	if (hmutex == TB_INVALID_HANDLE) return TB_FALSE;
