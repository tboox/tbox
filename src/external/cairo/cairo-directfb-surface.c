					 ...
					 int              height)
{
    ...
    surface = tb_calloc (1, sizeof (cairo_directfb_surface_t));
			       ...
			       cairo_directfb_font_cache_t **out)
{
    ...
    cache = tb_calloc (1, sizeof (cairo_directfb_font_cache_t));
    if (cache == NULL)
        return _cairo_error (CAIRO_STATUS_NO_MEMORY);
			      ...
			      int                          *ret_num)
{
	...
	if (scaled_glyph->surface_private == NULL) {
	    ...
	    rect = tb_malloc (sizeof (DFBRectangle));
	    if (rect == NULL) {
		_cairo_scaled_font_thaw_cache (scaled_font);
		return _cairo_error (CAIRO_STATUS_NO_MEMORY);
	    }
                                           ...
                                           cairo_scaled_font_t  *scaled_font)
{
     ...
     if (scaled_glyph->surface_private != NULL) {
          tb_free (scaled_glyph->surface_private);
