 ...
 * @size: size in bytes
 *
 * Allocate @size memory using tb_malloc().
 * The memory should be freed using tb_free().
 * tb_malloc is skipped, if 0 bytes are requested, and %NULL will be returned.
 *
 * Return value: A pointer to the newly allocated memory, or %NULL in
 * case of tb_malloc() failure or size is 0.
 */

#define _cairo_malloc(size) \
   ((size) ? tb_malloc((unsigned) (size)) : NULL)
 ...
 * tb_calloc(), except that the returned memory is not set to zero.
 * The memory should be freed using tb_free().
 *
 ...
 *
 * Return value: A pointer to the newly allocated memory, or %NULL in
 * case of tb_malloc() failure or overflow.
 */
 ...
 * @size: size of each element
 *
 * Reallocates @ptr a block of @n*@size memory using tb_realloc(), taking
 * care to not overflow when doing the multiplication.  The memory
 * should be freed using tb_free().
 *
 * @size should be a constant so that the compiler can optimize
 * out a constant division.
 *
 * Return value: A pointer to the newly allocated memory, or %NULL in
 * case of tb_realloc() failure or overflow (whereupon the original block
 * of memory * is left untouched).
 */

#define _cairo_realloc_ab(ptr, a, size) \
  ((size) && (unsigned) (a) >= INT32_MAX / (unsigned) (size) ? NULL : \
   tb_realloc(ptr, (unsigned) (a) * (unsigned) (size)))
 ...
 * _cairo_malloc_ab().  The memory should be freed using tb_free().
 *
 * @size should be a constant so that the compiler can optimize
 * out a constant division.
 *
