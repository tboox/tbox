...
_lzw_buf_grow (lzw_buf_t *buf)
{
	...
	new_data = tb_realloc (buf->data, new_size);
 ...
 * use a zero value to represent tb_free slots in the hash table. */
#define LZW_SYMBOL_FREE		0x00000000

/* These really aren't very tb_free for modifying. First, the PostScript
 ...
 * Reference. The tb_free parameters in the LZW algorithm are set to the
