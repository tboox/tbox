                       ...
                       unsigned int length)
{
    ...
    element.data = tb_malloc (element.length);
			  ...
			  cff_dict_operator_t **out)
{
    ...
    op = tb_malloc (sizeof (cff_dict_operator_t));
    if (unlikely (op == NULL))
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);

    _cairo_dict_init_key (op, operator);
    op->operand = tb_malloc (size);
...
cairo_cff_font_read_fdselect (cairo_cff_font_t *font, unsigned char *p)
{
    ...
    font->fdselect = tb_calloc (font->num_glyphs, sizeof (int));
...
cairo_cff_font_read_cid_fontdict (cairo_cff_font_t *font, unsigned char *ptr)
{
    ...
    font->fd_dict = tb_calloc (sizeof (cairo_hash_table_t *), font->num_fontdicts);
    if (unlikely (font->fd_dict == NULL)) {
        status = _cairo_error (CAIRO_STATUS_NO_MEMORY);
        goto fail;
    }

    font->fd_private_dict = tb_calloc (sizeof (cairo_hash_table_t *), font->num_fontdicts);
    if (unlikely (font->fd_private_dict == NULL)) {
        status = _cairo_error (CAIRO_STATUS_NO_MEMORY);
        goto fail;
    }

    font->fd_local_sub_index = tb_calloc (sizeof (cairo_array_t), font->num_fontdicts);
...
cairo_cff_font_subset_fontdict (cairo_cff_font_t  *font)
{
    ...
    font->fdselect_subset = tb_calloc (font->scaled_font_subset->num_glyphs,
                                     sizeof (int));
    if (unlikely (font->fdselect_subset == NULL))
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);

    font->fd_subset_map = tb_calloc (font->num_fontdicts, sizeof (int));
    if (unlikely (font->fd_subset_map == NULL))
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);

    font->private_dict_offset = tb_calloc (font->num_fontdicts, sizeof (int));
    if (unlikely (font->private_dict_offset == NULL))
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);

    reverse_map = tb_calloc (font->num_fontdicts, sizeof (int));
...
cairo_cff_font_create_cid_fontdict (cairo_cff_font_t *font)
{
    ...
    font->fd_dict = tb_malloc (sizeof (cairo_hash_table_t *));
    if (unlikely (font->fd_dict == NULL))
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);

    if (cff_dict_init (&font->fd_dict[0])) {
	tb_free (font->fd_dict);
	font->fd_dict = NULL;
	font->num_fontdicts = 0;
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);
    }

    font->fd_subset_map = tb_malloc (sizeof (int));
    if (unlikely (font->fd_subset_map == NULL))
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);

    font->private_dict_offset = tb_malloc (sizeof (int));
    if (unlikely (font->private_dict_offset == NULL))
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);
                        ...
                        const char                  *subset_name)
{
    ...
    font = tb_malloc (sizeof (cairo_cff_font_t));
    ...
    font->widths = tb_calloc (font->scaled_font_subset->num_glyphs, sizeof (int));
    ...
    font->data = tb_malloc (data_length);
...
fail4:
    if (font->font_name)
	tb_free (font->font_name);
fail3:
    tb_free (font->subset_font_name);
fail2:
                        ...
                        cairo_scaled_font_subset_t  *font_subset)
{
    ...
    cff_subset->widths = tb_calloc (sizeof (int), font->scaled_font_subset->num_glyphs);
    ...
    cff_subset->data = tb_malloc (length);
 ...
 fail4:
    tb_free (cff_subset->widths);
 fail3:
    if (cff_subset->font_name)
	tb_free (cff_subset->font_name);
 fail2:
    tb_free (cff_subset->ps_name);
 fail1:
    cairo_cff_font_destroy (font);
...
{
    tb_free (subset->ps_name);
    if (subset->font_name)
	tb_free (subset->font_name);
    tb_free (subset->widths);
    tb_free (subset->data);
}
