...
{
    pixman_image_t *image = tb_malloc (sizeof (pixman_image_t));
...
pixman_image_unref (pixman_image_t *image)
{
	...
	if (common->transform)
	    tb_free (common->transform);

	if (common->filter_params)
	    tb_free (common->filter_params);
	    ...
	    image->type == CONICAL)
	{
		...
		tb_free (image->gradient.stops);
	}

	if (image->type == BITS && image->bits.free_me)
	    tb_free (image->bits.free_me);
                            ...
                            const pixman_transform_t *transform)
{
	...
	common->transform = tb_malloc (sizeof (pixman_transform_t));
