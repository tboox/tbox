...
_cairo_ft_unscaled_font_map_create (void)
{
    ...
    font_map = tb_malloc (sizeof (cairo_ft_unscaled_font_map_t));
					 ...
					 cairo_ft_unscaled_font_t **out)
{
    ...
    unscaled = tb_malloc (sizeof (cairo_ft_unscaled_font_t));
		     ...
		     cairo_image_surface_t  **surface)
{
	...
	*surface = (cairo_image_surface_t *)
	...
	case CAIRO_ANTIALIAS_SUBPIXEL: {
	    ...
	    data_rgba = tb_calloc (stride_rgba, height);
	    if (unlikely (data_rgba == NULL)) {
		if (own_buffer)
		    tb_free (bitmap->buffer);
		return _cairo_error (CAIRO_STATUS_NO_MEMORY);
	    }
		       ...
		       cairo_image_surface_t	**surface)
{
	...
	bitmap.buffer = tb_calloc (stride, bitmap.rows);
					...
					cairo_scaled_font_t       **font_out)
{
    ...
    scaled_font = tb_malloc (sizeof (cairo_ft_scaled_font_t));
					...
					cairo_font_face_t **out)
{
    ...
    font_face = tb_malloc (sizeof (cairo_ft_font_face_t));
 ...
 * if it needs to.  You are tb_free to modify or tf_free @pattern after this call.
 *
 ...
 *   is safe to tb_free the face is to pass a
