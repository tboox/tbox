 ...
 * XXX fragmentation? use Doug Lea's tb_malloc?
 */
	   ...
	   cairo_bool_t clear)
{
    ...
    cairo_list_add (&block->link, &pi->tb_free[bits]);
...
/* Add a chunk to the tb_free list */
static void
			      ...
			      unsigned int num_sizes)
{
    ...
    assert (num_sizes < ARRAY_LENGTH (pi->tb_free));
    ...
    pi->blocks = tb_calloc (setBits, sizeof (cairo_xcb_shm_mem_block_t));
    if (pi->blocks == NULL)
    ...
    for (i = 0; i < ARRAY_LENGTH (pi->tb_free); i++)
	cairo_list_init (&pi->tb_free[i]);

    pi->map = tb_malloc ((setBits + 7) >> 3);
    if (pi->map == NULL) {
	tb_free (pi->blocks);
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);
    }
    ...
    /* Now add all blocks to the tb_free list */
	      ...
	      unsigned int bits)
{
	...
	if (! cairo_list_is_empty (&pi->tb_free[b])) {
	    block = cairo_list_first_entry (&pi->tb_free[b],
					    cairo_xcb_shm_mem_block_t,
					    link);
	    break;
	}
    ...
    while (cairo_list_is_empty (&pi->tb_free[pi->max_free_bits])) {
    ...
    /* If we used a larger tb_free block than we needed, tf_free the rest */
					 ...
					 cairo_xcb_shm_info_t **shm_info_out)
{
    ...
    pool = tb_malloc (sizeof (cairo_xcb_shm_mem_pool_t));
