			       ...
			       cairo_bool_t                hex_encode)
{
    ...
    font->glyphs = tb_calloc (face->num_glyphs, sizeof font->glyphs[0]);
...
cairo_type1_font_subset_decrypt_eexec_segment (cairo_type1_font_subset_t *font)
{
    ...
    font->cleartext = tb_malloc (font->eexec_segment_size);
				      ...
				      const char *encrypted_charstring, int encrypted_charstring_length)
{
    ...
    charstring = tb_malloc (encrypted_charstring_length);
...

{
    ...
    font->type1_data = tb_malloc (font->type1_length);
...
_cairo_type1_font_subset_fini (cairo_type1_font_subset_t *font)
{
    ...
    tb_free (font->type1_data);
    if (font->glyphs != NULL) {
	for (i = 0; i < font->base.num_glyphs; i++)
	    tb_free (font->glyphs[i].name);
    }
                          ...
                          cairo_bool_t                   hex_encode)
{
    ...
    type1_subset->widths = tb_calloc (sizeof (int), font.num_glyphs);
    ...
    length = font.base.header_size +
	     font.base.data_size +
	     font.base.trailer_size;
    type1_subset->data = tb_malloc (length);
 ...
 fail3:
    tb_free (type1_subset->widths);
 fail2:
    tb_free (type1_subset->base_font);
 fail1:
