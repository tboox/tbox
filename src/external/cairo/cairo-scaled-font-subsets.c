                              ...
                              double            y_advance)
{
    ...
    sub_font_glyph = tb_malloc (sizeof (cairo_sub_font_glyph_t));
...
_cairo_sub_font_glyph_destroy (cairo_sub_font_glyph_t *sub_font_glyph)
{
	...
	tb_free (sub_font_glyph->utf8);

    tb_free (sub_font_glyph);
}
			...
			cairo_sub_font_t               **sub_font_out)
{
    ...
    sub_font = tb_malloc (sizeof (cairo_sub_font_t));
				      ...
				      unsigned long	      scaled_font_glyph_index)
{
	...
	if (len > 0) {
	    sub_font_glyph->utf8 = tb_malloc (len + 1);
    ...
    *is_mapped = FALSE;
	...
	if (sub_font_glyph->utf8 != NULL) {
		...
		*is_mapped = TRUE;
	    ...
	    sub_font_glyph->utf8 = tb_malloc (utf8_len + 1);
	    if (unlikely (sub_font_glyph->utf8 == NULL))
		return _cairo_error (CAIRO_STATUS_NO_MEMORY);
...
_cairo_sub_font_collect (void *entry, void *closure)
{
	...
	subset.num_glyphs = collection->num_glyphs;
        ...
        if (subset.to_unicode != NULL)
            tb_free (subset.to_unicode);

	if (subset.glyph_names != NULL) {
            for (j = 0; j < collection->num_glyphs; j++)
		tb_free (subset.glyph_names[j]);
	    tb_free (subset.glyph_names);
	}
...
_cairo_scaled_font_subsets_create_internal (cairo_subsets_type_t type)
{
    ...
    subsets = tb_malloc (sizeof (cairo_scaled_font_subsets_t));
					     ...
					     cairo_subsets_foreach_type_t	       type)
{
	...
	if (collection.glyphs != NULL)
	    tb_free (collection.glyphs);
	if (collection.utf8 != NULL)
	    tb_free (collection.utf8);
...
{
    *entry = tb_malloc (sizeof (cairo_string_entry_t));
...
_cairo_scaled_font_subset_create_glyph_names (cairo_scaled_font_subset_t *subset)
{
    ...
    subset->glyph_names = tb_calloc (subset->num_glyphs, sizeof (char *));
