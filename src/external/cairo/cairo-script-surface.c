		 ...
		 unsigned long *id)
{
    ...
    bb = tb_malloc (sizeof (struct _bitmap));
    if (unlikely (bb == NULL))
	    ...
	    cairo_bool_t force)
{
	...
	if (surface->cr.current_style.dash != NULL) {
	    tb_free (surface->cr.current_style.dash);
...
_cairo_script_surface_finish (void *abstract_surface)
{
	...
	if (! cairo_list_is_empty (&surface->operand.link)) {
		...
		struct deferred_finish *link = tb_malloc (sizeof (*link));
		if (link == NULL) {
			...
			cairo_scaled_font_t *scaled_font)
{
    ...
    font_private = tb_malloc (sizeof (cairo_script_surface_font_private_t));
				       ...
				       cairo_surface_t *passthrough)
{
    ...
    surface = tb_malloc (sizeof (cairo_script_surface_t));
...
_cairo_script_context_create_internal (cairo_output_stream_t *stream)
{
    ...
    ctx = tb_malloc (sizeof (cairo_script_context_t));
    if (unlikely (ctx == NULL))
