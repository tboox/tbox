...
_cairo_clip_path_create (cairo_clip_t *clip)
{
	...
	clip_path = tb_malloc (sizeof (cairo_clip_path_t));
	if (unlikely (clip_path == NULL))
	    return NULL;
    }
...
#define ERROR_LIST(S) _cairo_rectangle_list_create_in_error (_cairo_error (S));
	...
	n_rects = 1;
	rectangles = tb_malloc(sizeof (cairo_rectangle_t));
	if (unlikely (rectangles == NULL))
	    return ERROR_LIST (CAIRO_STATUS_NO_MEMORY);
 ...
 DONE:
    list = tb_malloc (sizeof (cairo_rectangle_list_t));
    if (unlikely (list == NULL)) {
        tb_free (rectangles);
	return ERROR_LIST (CAIRO_STATUS_NO_MEMORY);
    }
...
cairo_rectangle_list_destroy (cairo_rectangle_list_t *rectangle_list)
{
    ...
    tb_free (rectangle_list->rectangles);
    tb_free (rectangle_list);
}
