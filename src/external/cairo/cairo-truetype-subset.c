			     ...
			     cairo_truetype_font_t      **font_return)
{
    ...
    font = tb_malloc (sizeof (cairo_truetype_font_t));
    ...
    font->glyphs = tb_calloc (font->num_glyphs_in_face + 1, sizeof (subset_glyph_t));
    if (unlikely (font->glyphs == NULL)) {
	status = _cairo_error (CAIRO_STATUS_NO_MEMORY);
	goto fail1;
    }

    font->parent_to_subset = tb_calloc (font->num_glyphs_in_face, sizeof (int));
    ...
    if (font->base.ps_name == NULL) {
        font->base.ps_name = tb_malloc (30);
    ...
    font->base.widths = tb_calloc (font->num_glyphs_in_face, sizeof (int));
 ...
 fail5:
    ...
    tb_free (font->base.widths);
 fail4:
    tb_free (font->base.ps_name);
 fail3:
    tb_free (font->parent_to_subset);
    if (font->base.font_name)
	tb_free (font->base.font_name);
 fail2:
    tb_free (font->glyphs);
 fail1:
			     ...
			     cairo_scaled_font_subset_t	*font_subset)
{
    ...
    truetype_subset->widths = tb_calloc (sizeof (double),
    ...
    if (length) {
	truetype_subset->data = tb_malloc (length);
	...
	truetype_subset->string_offsets = tb_malloc (offsets_length);
 ...
 fail5:
    tb_free (truetype_subset->data);
 fail4:
    tb_free (truetype_subset->widths);
 fail3:
    if (truetype_subset->font_name)
	tb_free (truetype_subset->font_name);
 fail2:
    tb_free (truetype_subset->ps_name);
 fail1:
    cairo_truetype_font_destroy (font);
				...
				char 	       		**font_name_out)
{
   ...
   status = backend->load_truetype_table (scaled_font,
    ...
    for (i = 0; i < be16_to_cpu(name->num_records); i++) {
		...
		font_name = tb_malloc (be16_to_cpu(record->length) + 1);
		if (font_name == NULL) {
		    status = _cairo_error (CAIRO_STATUS_NO_MEMORY);
		    goto fail;
		}
		...
		ps_name = tb_malloc (be16_to_cpu(record->length) + 1);
		if (ps_name == NULL) {
		    status = _cairo_error (CAIRO_STATUS_NO_MEMORY);
		    goto fail;
		}
