                                                    ...
                                                    cairo_linear_pattern_t *pattern)
{
    ...
    vert = tb_malloc (sizeof (TRIVERTEX) * (num_rects*2*num_ranges + 4));
    rect = tb_malloc (sizeof (GRADIENT_RECT) * (num_rects*num_ranges + 2));
				      ...
				      cairo_clip_t    *clip)
{
	...
	dash_array = tb_calloc (sizeof (DWORD), style->num_dashes);
...
cairo_win32_printing_surface_create (HDC hdc)
{
    ...
    surface = tb_malloc (sizeof (cairo_win32_surface_t));
    if (surface == NULL)
