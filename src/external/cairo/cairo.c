...
#if HAS_ATOMIC_OPS
/* We keep a small stash of contexts to reduce tb_malloc pressure */
...
_context_get (void)
{
	...
	if (avail >= CAIRO_STASH_SIZE)
	    return tb_malloc (sizeof (cairo_t));
...
#else
#define _context_get() tb_malloc (sizeof (cairo_t))
#define _context_put(cr) tb_free (cr)
#endif
