 ...
 * nodes, a new one will be allocated using tb_malloc().  The caller is
 * responsible for calling _cairo_freelist_free() or tb_free() on the
 * returned node.  Returns %NULL on memory allocation error. */
cairo_private void *
_cairo_freelist_alloc (cairo_freelist_t *freelist);

/* Allocate a new node from the freelist.  If the freelist contains no
 * nodes, a new one will be allocated using tb_calloc().  The caller is
 * responsible for calling _cairo_freelist_free() or tb_free() on the
 * returned node.  Returns %NULL on memory allocation error. */
cairo_private void *
