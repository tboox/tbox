					       ...
					       double			 height)
{
    ...
    surface = tb_malloc (sizeof (cairo_pdf_surface_t));
...
_cairo_pdf_surface_create_smask_group (cairo_pdf_surface_t	*surface)
{
    ...
    group = tb_calloc (1, sizeof (cairo_pdf_smask_group_t));
...
_cairo_pdf_smask_group_destroy (cairo_pdf_smask_group_t *group)
{
	...
	tb_free (group->utf8);
    if (group->glyphs)
	tb_free (group->glyphs);
    if (group->clusters)
	tb_free (group->clusters);
				       ...
				       int                      *height)
{
	...
	*height = surface_entry->height;
    ...
    surface_entry = tb_malloc (sizeof (cairo_pdf_source_surface_entry_t));
