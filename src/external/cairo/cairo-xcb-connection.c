					  ...
					  const xcb_render_query_pict_formats_reply_t *formats)
{
	     ...
	     xcb_render_pictdepth_next (&depths))
	{
		...
		f = tb_malloc (sizeof (cairo_xcb_xrender_format_t));
		if (unlikely (f == NULL))
...
_device_finish (void *device)
{
	 ...
	 * a later use-after-tb_free on shutdown of the connection.
	 */
...
_cairo_xcb_connection_get (xcb_connection_t *xcb_connection)
{
    ...
    connection = tb_malloc (sizeof (cairo_xcb_connection_t));
