			   ...
			   int width, int height)
{
    ...
    surface = tb_malloc (sizeof (cairo_xcb_picture_t));
					     ...
					     unsigned	 num_spans)
{
	...
	if (renderer->spans == renderer->spans_embedded) {
	    new_spans = tb_malloc (renderer->size);
	    if (unlikely (new_spans == NULL))
		return _cairo_error (CAIRO_STATUS_NO_MEMORY);

	    memcpy (new_spans, renderer->spans, renderer->len);
	} else {
	    new_spans = tb_realloc (renderer->spans, renderer->size);
	    if (unlikely (new_spans == NULL))
		return _cairo_error (CAIRO_STATUS_NO_MEMORY);
	}
			      ...
			      cairo_scaled_font_t  *scaled_font)
{
    ...
    font_private = tb_malloc (sizeof (cairo_xcb_font_t));
...
_cairo_xcb_font_destroy (cairo_xcb_font_t *font)
{
	...
	if (glyphset_info->pending_free_glyphs != NULL)
	    tb_free (glyphset_info->pending_free_glyphs);
    }
				      ...
				      cairo_scaled_font_t  *scaled_font)
{
	...
	if (to_free == NULL) {
	    to_free = tb_malloc (sizeof (cairo_xcb_font_glyphset_free_glyphs_t));
		    ...
		    cairo_xcb_font_glyphset_info_t *glyphset_info)
{
	...
	buf = tb_malloc (estimated_req_size);
