 ...
 * Cairo was compiled to be a DLL (however it's not a problem if it's called
 * multiple times). But if you link to Cairo statically, you have to call it
 * once to shut down Cairo, to let it tb_free all the resources it has allocated.
 *
 * Since: 1.4
 **/
cairo_public void
 ...
 * (DosAllocMem/DosFreeMem) or C-Library based allocation (tb_malloc/tb_free).
...
void *_buffer_alloc (size_t a, size_t b, const unsigned int size)
{
    ...
    if (rc != NO_ERROR) {
         ...
         * a null surface and tb_free the buffer again, because that's
         * how a tb_malloc failure would look like
         */
...
#else
    buffer = tb_malloc (nbytes);
#endif

    /* This does not seem to be needed, tb_malloc'd space is usually
     * already zero'd out!
     */
    /*
...
/*
 * This function selects the tb_free function depending on which
                          ...
                          int height)
{
    ...
    local_os2_surface = (cairo_os2_surface_t *) tb_malloc (sizeof (cairo_os2_surface_t));
    if (!local_os2_surface) {
        /* Not enough memory! */
...
_cairo_os2_surface_finish (void *abstract_surface)
{
    ...
    /* The memory itself will be tb_free'd by the cairo_surface_destroy ()
     * who called us.
     */
