 ...
 * needs to allocate an array of glyphs that cairo will tb_free.
...
/**
 * cairo_glyph_free:
 * @glyphs: array of glyphs to tb_free, or %NULL
 *
 * Frees an array of #cairo_glyph_t's allocated using cairo_glyph_allocate().
 * This function is only useful to tb_free glyph array returned
 * by cairo_scaled_font_text_to_glyphs() where cairo returns
 * an array of glyphs that the user will tb_free.
 * For all other uses, user can use their own allocation method
 * for glyphs.
 *
 * Since: 1.8
 ...
 * This function is only useful in implementations of
 * #cairo_user_scaled_font_text_to_glyphs_func_t where the user
 * needs to allocate an array of text clusters that cairo will tb_free.
 * For all other uses, user can use their own allocation method
 * for text clusters.
 *
 ...
 * @clusters: array of text clusters to tb_free, or %NULL
 *
 * Frees an array of #cairo_text_cluster's allocated using cairo_text_cluster_allocate().
 * This function is only useful to tb_free text cluster array returned
 * by cairo_scaled_font_text_to_glyphs() where cairo returns
 * an array of text clusters that the user will tb_free.
 * For all other uses, user can use their own allocation method
 * for text clusters.
 *
 * Since: 1.8
...
_cairo_intern_string (const char **str_inout, int len)
{
    ...
    istring = _cairo_hash_table_lookup (_cairo_intern_string_ht,
					&tmpl.hash_entry);
    if (istring == NULL) {
	istring = tb_malloc (sizeof (cairo_intern_string_t) + len + 1);
