				...
				const cairo_rectangle_t	*extents)
{
    ...
    recording_surface = tb_malloc (sizeof (cairo_recording_surface_t));
...
_cairo_recording_surface_finish (void *abstract_surface)
{
	...
	case CAIRO_COMMAND_SHOW_TEXT_GLYPHS:
	    ...
	    tb_free (command->show_text_glyphs.utf8);
	    tb_free (command->show_text_glyphs.glyphs);
	    tb_free (command->show_text_glyphs.clusters);
				...
				cairo_clip_t		  *clip)
{
    ...
    command = tb_malloc (sizeof (cairo_command_paint_t));
			       ...
			       cairo_clip_t		*clip)
{
    ...
    command = tb_malloc (sizeof (cairo_command_mask_t));
    if (unlikely (command == NULL))
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);
				 ...
				 cairo_clip_t		*clip)
{
    ...
    command = tb_malloc (sizeof (cairo_command_stroke_t));
    if (unlikely (command == NULL))
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);
			       ...
			       cairo_clip_t		*clip)
{
    ...
    command = tb_malloc (sizeof (cairo_command_fill_t));
    if (unlikely (command == NULL))
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);
					   ...
					   cairo_clip_t			*clip)
{
    ...
    command = tb_malloc (sizeof (cairo_command_show_text_glyphs_t));
    if (unlikely (command == NULL))
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);
    ...
    if (utf8_len) {
	command->utf8 = tb_malloc (utf8_len);
  ...
  CLEANUP_ARRAYS:
    tb_free (command->utf8);
    tb_free (command->glyphs);
    tb_free (command->clusters);
