...
CreateGradientFunction (const cairo_gradient_pattern_t *gpat)
{
	...
	/* quartz doesn't deal very well with tb_malloc failing, so there's
			   ...
			   CGImageRef *image_out)
{
	...
	*image_out = CGImageRetain (surface->image);
    ...
    source_img = tb_malloc (sizeof (quartz_source_image_t));
    if (source_img == NULL)
							 ...
							 CGPatternRef *cgpat)
{
    ...
    info = tb_malloc(sizeof(SurfacePatternDrawInfo));
    if (!info)
					...
					unsigned int height)
{
    ...
    surface = tb_malloc(sizeof(cairo_quartz_surface_t));
    if (surface == NULL)
			     ...
			     unsigned int height)
{
     ...
     * pointer should be 16-byte aligned.  tb_malloc already aligns to 16-bytes,
