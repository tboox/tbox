...
encode_mcu_AC_first (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (ke = cinfo->Se + 1; ke > 1; ke--)
     ...
     */
    if ((v = (*block)[tb_jpeg_natural_order[ke - 1]]) >= 0) {
      if (v >>= cinfo->Al) break;
    } else {
      v = -v;
      if (v >>= cinfo->Al) break;
  ...
  for (k = cinfo->Ss; k < ke; k++) {
    ...
    for (;;) {
      if ((v = (*block)[tb_jpeg_natural_order[k]]) >= 0) {
	if (v >>= cinfo->Al) {
...
encode_mcu_AC_refine (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (kex = ke; kex > 1; kex--)
    if ((v = (*block)[tb_jpeg_natural_order[kex - 1]]) >= 0) {
      if (v >>= cinfo->Ah) break;
    } else {
      v = -v;
      if (v >>= cinfo->Ah) break;
...
encode_mcu (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    for (ke = DCTSIZE2; ke > 1; ke--)
      if ((*block)[tb_jpeg_natural_order[ke - 1]]) break;

    /* Figure F.5: Encode_AC_Coefficients */
    for (k = 1; k < ke; k++) {
      st = entropy->ac_stats[tbl] + 3 * (k - 1);
      arith_encode(cinfo, st, 0);	/* EOB decision */
      while ((v = (*block)[tb_jpeg_natural_order[k]]) == 0) {
	arith_encode(cinfo, st + 1, 0); st += 3; k++;
      }
      arith_encode(cinfo, st + 1, 1);
      /* Figure F.6: Encoding nonzero value v */
...
GLOBAL(void)
tb_jpeg_jinit_arith_encoder (j_compress_ptr cinfo)
{
