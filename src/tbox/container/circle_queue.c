...
static tb_long_t tb_circle_queue_itor_comp(tb_iterator_ref_t iterator, tb_cpointer_t ltem, tb_cpointer_t rtem)
{
    // check
    tb_circle_queue_impl_t* impl = (tb_circle_queue_impl_t*)iterator;
...
tb_bool_t tb_circle_queue_load(tb_circle_queue_ref_t circle_queue, tb_stream_ref_t stream)
{
    // check
    tb_circle_queue_impl_t* impl = (tb_circle_queue_impl_t*)circle_queue;
    ...
    {
        // calc type
        crc32 = tb_crc_encode_cstr(TB_CRC_MODE_32_IEEE_LE, crc32, "circle_queue");
        ...
        for (load = 0; load < size; load++)
        {
            ...
            // save item
            tb_circle_queue_put(circle_queue, data);
...
tb_bool_t tb_circle_queue_save(tb_circle_queue_ref_t circle_queue, tb_stream_ref_t stream)
{
    // check
    tb_circle_queue_impl_t* impl = (tb_circle_queue_impl_t*)circle_queue;
    tb_assert_and_check_return_val(impl && stream, tb_false);
    ...
    do
    {
        ...
        // the size
        tb_size_t size = tb_circle_queue_size(circle_queue);
        ...
        // save circle_queue
        tb_size_t save = 0;
        tb_for_all (tb_cpointer_t, item, circle_queue)
        {
            // save item
