...
static tb_void_t tb_fixed_pool_slot_exit(tb_fixed_pool_impl_t* impl, tb_fixed_pool_slot_t* slot)
{
    ...
    tb_iterator_ref_t   iterator = tb_iterator_make_for_ptr(&array_iterator, (tb_pointer_t*)impl->slot_list, impl->slot_count);
...
tb_void_t tb_fixed_pool_exit(tb_fixed_pool_ref_t pool)
{
    ...
    // exit the slot list
    if (impl->slot_list) tb_large_pool_free(impl->large_pool, impl->slot_list);
