 ...
 * }
 *
 * tb_rfor(tb_hash_map_element_ref_t, item, tb_iterator_head(iterator), tb_iterator_tail(iterator), iterator)
 * {
 ...
 * }
 *
 * tb_rfor_all(tb_hash_map_element_ref_t, item, iterator)
 * {
 *      if (item) tb_trace_d("item: %p => %p", item->name, item->data);
 * }
 * @endcode
