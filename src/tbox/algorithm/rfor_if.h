 ...
 * }
 *
 * tb_rfor_if(tb_hash_map_element_ref_t, item, tb_iterator_head(iterator), tb_iterator_tail(iterator), iterator, item != tb_null)
 * {
 ...
 * }
 *
 * tb_rfor_all(tb_hash_map_element_ref_t, item, iterator, item != tb_null)
 * {
 *      tb_trace_d("item: %p => %p", item->name, item->data);
 * }
 * @endcode
