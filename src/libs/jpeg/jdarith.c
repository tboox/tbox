...
{
  struct tb_jpeg_source_mgr * src = cinfo->src;
...
decode_mcu_DC_first (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    /* Scale and output the DC coefficient (assumes tb_jpeg_natural_order[0]=0) */
...
decode_mcu_AC_first (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (k = cinfo->Ss; k <= cinfo->Se; k++) {
    ...
    (*block)[tb_jpeg_natural_order[k]] = (JCOEF) (v << cinfo->Al);
  }
...
decode_mcu_AC_refine (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (kex = cinfo->Se + 1; kex > 1; kex--)
    if ((*block)[tb_jpeg_natural_order[kex - 1]]) break;
...
decode_mcu (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
	...
	if (arith_decode(cinfo, st)) v |= m;
      v += 1; if (sign) v = -v;
      (*block)[tb_jpeg_natural_order[k]] = (JCOEF) v;
    }
  }
...
GLOBAL(void)
tb_jpeg_jinit_arith_decoder (j_decompress_ptr cinfo)
{
