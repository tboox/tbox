...
tb_int_t main(tb_int_t argc, tb_char_t** argv)
{
	...
	tb_gstream_t* nst = tb_gstream_init_from_null(ist);

	if (!ist || !ost || !nst) goto end;
	...
	if (!tb_gstream_bopen(nst)) goto end;
	
#if 0
	// save stream
	tb_uint64_t size = tb_gstream_save(nst, ost);
	tb_print("save: %llu bytes", size);
#else
	// read data
	tb_byte_t 		data[TB_GSTREAM_BLOCK_MAXN];
	tb_uint64_t 	read = 0;
	tb_uint64_t 	left = tb_gstream_left(nst);
	tb_int64_t 		base = tb_mclock();
	tb_int64_t 		basc = tb_mclock();
	do
	{
		// read data
		tb_long_t n = tb_gstream_aread(nst, data, TB_GSTREAM_BLOCK_MAXN);
//		tb_trace("read: %d, offset: %llu, left: %llu, size: %llu", n, tb_gstream_offset(ist), tb_gstream_left(ist), tb_gstream_size(ist));
		...
		{
			// wait
			tb_long_t e = tb_gstream_wait(ist, TB_AIOO_ETYPE_READ, tb_gstream_timeout(nst));
