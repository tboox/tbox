 ...
 * implementation
 */ 
static tb_bool_t tb_demo_async_stream_cache_done_func(tb_size_t state, tb_hize_t offset, tb_hong_t size, tb_hize_t save, tb_size_t rate, tb_cpointer_t priv)
{
    // trace
...
//      tb_async_transfer_limitrate(transfer, 4096);
        ...
        if (!tb_async_transfer_open_done(transfer, 0, tb_demo_async_stream_cache_done_func, event)) break;
