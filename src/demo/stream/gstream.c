...
tb_int_t tb_demo_stream_gstream_main(tb_int_t argc, tb_char_t** argv)
{
	...
	do
	{
		...
		if (tb_option_done(option, argc - 1, &argv[1]))
		{
			...
			{
				// init istream
				istream = tb_gstream_init_from_url(tb_option_item_cstr(option, "url"));
				...
				if (tb_stream_type(istream) == TB_STREAM_TYPE_HTTP) 
				{
					...
					// enable debug?
					if (!tb_stream_ctrl(istream, TB_STREAM_CTRL_HTTP_SET_HEAD_FUNC, debug? tb_demo_gstream_head_func : tb_null)) break;
							...
							else
							{
								...
								else
								{
									...
									if (tb_pstring_size(&key) && tb_pstring_size(&val))
									{
										...
										if (!tb_stream_ctrl(istream, TB_STREAM_CTRL_HTTP_SET_HEAD, tb_pstring_cstr(&key), tb_pstring_cstr(&val))) break;
									}
					...
					if (tb_option_find(option, "post-data"))
					{
						...
						if (!tb_stream_ctrl(istream, TB_STREAM_CTRL_HTTP_SET_POST_SIZE, post_size)) break;
				...
				if (tb_option_find(option, "more0"))
				{
					...
					// init
					ostream = tb_gstream_init_from_file(path, TB_FILE_MODE_RW | TB_FILE_MODE_CREAT | TB_FILE_MODE_BINARY | TB_FILE_MODE_TRUNC);
