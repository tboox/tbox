 ...
 * implementation
 */ 
static tb_bool_t tb_demo_async_stream_null_save_func(tb_size_t state, tb_hize_t offset, tb_hong_t size, tb_hize_t save, tb_size_t rate, tb_pointer_t priv)
{
	// trace
...
tb_int_t tb_demo_stream_async_stream_null_main(tb_int_t argc, tb_char_t** argv)
{
	...
	do
	{
		...
		// init tstream
		tstream = tb_transfer_stream_init_aa(fstream, ostream, 0);
		tb_assert_and_check_break(tstream);

		// open and save tstream
		if (!tb_transfer_stream_osave(tstream, tb_demo_async_stream_null_save_func, aicp)) break;
