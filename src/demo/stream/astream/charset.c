 ...
 * implementation
 */ 
static tb_bool_t tb_demo_astream_charset_save_func(tb_handle_t istream, tb_handle_t ostream, tb_size_t state, tb_hize_t size, tb_size_t rate, tb_pointer_t priv)
{
	// trace
...
//		tb_astream_t* iostream = ostream;
		...
		// init tstream
		if (iostream == istream) tstream = tb_tstream_init_aa(fstream, ostream, -1, tb_null, tb_demo_astream_charset_save_func, aicp);
		else tstream = tb_tstream_init_aa(istream, fstream, -1, tb_null, tb_demo_astream_charset_save_func, aicp);
