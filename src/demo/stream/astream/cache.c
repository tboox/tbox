...
//		tb_astream_t* iostream = ostream;
		...
		// init tstream
		if (iostream == istream) tstream = tb_tstream_init_aa(fstream, ostream, -1, tb_null, tb_demo_astream_cache_save_func, aicp);
		else tstream = tb_tstream_init_aa(istream, fstream, -1, tb_null, tb_demo_astream_cache_save_func, aicp);
