 ...
 * implementation
 */ 
static tb_bool_t tb_demo_astream_zip_save_func(tb_size_t state, tb_hize_t offset, tb_hong_t size, tb_hize_t save, tb_size_t rate, tb_pointer_t priv)
{
	// percent
	tb_size_t percent = 0;
	...
	// trace
	tb_print("save: %llu bytes, rate: %lu bytes/s, percent: %lu%%, state: %s", save, rate, percent, tb_stream_state_cstr(state));
