...
static tb_bool_t tb_single_list_test_walk_item(tb_single_list_t* list, tb_pointer_t item, tb_bool_t* bdel, tb_pointer_t priv)
{
	// check
...
static tb_void_t tb_single_list_test_walk_perf()
{
	...
	tb_hong_t t = tb_mclock();
	__tb_volatile__ tb_hize_t test[2] = {0};
	tb_single_list_walk(list, tb_single_list_test_walk_item, (tb_pointer_t)test);
	t = tb_mclock() - t;
	tb_trace_i("item: %llx, size: %llu ?= %u, time: %lld", test[0], test[1], tb_single_list_size(list), t);
