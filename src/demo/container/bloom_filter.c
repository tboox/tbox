...
static tb_void_t tb_demo_test_cstr_h(tb_size_t index)
{
    ...
    // save func
    g_func_indx = index;
    g_func_prev = tb_item_func_str(tb_true, tb_object_null);
...
static tb_void_t tb_demo_test_cstr_p()
{
    ...
    // init filter
    tb_bloom_filter_t* filter = tb_bloom_filter_init(TB_BLOOM_FILTER_PROBABILITY_0_01, 3, count, tb_item_func_str(tb_true, tb_object_null));
    if (filter)
    {
        // clear random
        tb_random_clear(tb_random_generator());
