...
static tb_pointer_t tb_test_thread(tb_pointer_t cb_data)
{
	...
	while (1)
	{
		...
		tb_size_t i = 0;
		for (i = 0; i < r; i++) 
			tb_trace_i("[event: %u]: signal", o[i].data);
	}

end:

	// exit thread
	tb_trace_i("[thread]: exit");
 ...
 * main
 */ 
tb_int_t tb_demo_platform_epool_main(tb_int_t argc, tb_char_t** argv)
{
	// init item
	tb_test_item_t it = {0};
