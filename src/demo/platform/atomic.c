...
static tb_void_t tb_test_atomic_compare_and_set()
{
    tb_trace_i("atomic_compare_and_set(): test ..");
    tb_atomic_t a;
    tb_atomic_init(&a, 0);
    tb_long_t p = 0;
    tb_assert_and_check_return(tb_atomic_compare_and_set(&a, &p, 1) && p == 0);
    p = 1;
    tb_assert_and_check_return(tb_atomic_compare_and_set(&a, &p, 2) && p == 1);
    p = 10;
    tb_assert_and_check_return(!tb_atomic_compare_and_set(&a, &p, 1000) && p == 2);
    tb_trace_i("atomic_compare_and_set(): test ok!");
}
...
{
    tb_test_atomic_get();
    tb_test_atomic_compare_and_set();
