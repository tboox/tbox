...
tb_int_t tb_demo_platform_ifaddrs_main(tb_int_t argc, tb_char_t** argv)
{
    ...
    // dump hwaddr
    tb_hwaddr_t hwaddr;
    if (tb_ifaddrs_hwaddr(tb_ifaddrs(), argv[1], &hwaddr))
    {
        // trace
    ...
    // dump ipaddr4
    tb_ipaddr_t ipaddr4;
    if (tb_ifaddrs_ipaddr(tb_ifaddrs(), argv[1], TB_IPADDR_FAMILY_IPV4, &ipaddr4))
    {
        // trace
    ...
    // dump ipaddr6
    tb_ipaddr_t ipaddr6;
    if (tb_ifaddrs_ipaddr(tb_ifaddrs(), argv[1], TB_IPADDR_FAMILY_IPV6, &ipaddr6))
    {
        // trace
