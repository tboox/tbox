...
{
    // init
    tb_static_fixed_pool_ref_t fpool = tb_static_fixed_pool_init((tb_byte_t*)malloc(50 * 1024 * 1024), 50 * 1024 * 1024, 64, 0);
    ...
    while (maxn--)
    {
        data = (__tb_volatile__ tb_byte_t*)tb_static_fixed_pool_malloc(fpool);
        tb_check_break(data);
    }
    time = tb_mclock() - time;
