...
{
    // init
    tb_handle_t pool = tb_static_block_pool_init((tb_byte_t*)malloc(50 * 1024 * 1024), 50 * 1024 * 1024, 0);
    ...
    while (maxn--)
    {
        ...
        data = (__tb_volatile__ tb_byte_t*)tb_static_block_pool_malloc(pool, (size & lmod)? (size & lmod) : 1);
        tb_check_break(data);
    }
    time = tb_mclock() - time;
#endif
