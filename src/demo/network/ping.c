...
static tb_bool_t tb_ping_recv(tb_socket_ref_t sock, tb_ipaddr_ref_t addr, tb_uint16_t seq)
{
    // recv echo 
    ...
    while (recv < size)
    {
        tb_long_t real = tb_socket_urecv(sock, addr, data + recv, size - recv);
        if (real > 0) 
        {
            recv += real;
            wait = tb_false;
...
tb_int_t tb_demo_network_ping_main(tb_int_t argc, tb_char_t** argv)
{
    ...
    if (sock)
    {
        ...
        {
            // recv ping
            if (!tb_ping_recv(sock, &addr, i)) break;
            i++;
