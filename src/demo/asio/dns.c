...
static tb_void_t tb_demo_sock_dns_done_func(tb_handle_t dns, tb_char_t const* host, tb_ipv4_t const* addr, tb_cpointer_t priv)
{
    ...
    // exit addr
    if (dns) tb_aicp_dns_exit(dns, tb_demo_sock_dns_exit_func, tb_object_null);
...
tb_int_t tb_demo_asio_dns_main(tb_int_t argc, tb_char_t** argv)
{
    ...
    tb_handle_t         dns = tb_object_null;
    ...
    // done dns
    tb_aicp_dns_done(dns, argv[1], tb_demo_sock_dns_done_func, tb_object_null);
