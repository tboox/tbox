...
tb_bool_t tb_flv_spak(tb_handle_t hflv)
{
    ...
    {
        // read flv header
        if (!tb_stream_bread(stream, tag, 9)) goto end;
    ...
    {
        // read flv tag
        if (!tb_stream_bread(stream, tag, 15)) goto end;
        ...
        case TB_FLV_TAG_TYPE_SDATA: // script data  
            {
                    ...
                    && !(flv->spank_type_ok & TB_FLV_SPANK_TYPE_SDATA)) 
                {
                    ...
                    // read data
                    if (!tb_stream_bread(stream, data, data_size)) goto end;
                ...
                {
                    // skip it
                    if (!tb_stream_skip(stream, data_size)) goto end;
                }
            }
            break; 
        case TB_FLV_TAG_TYPE_AUDIO: // audio data
            {
                ...
                if (flv->spank_type & (TB_FLV_SPANK_TYPE_AUDIO_CONFIG | TB_FLV_SPANK_TYPE_AUDIO_DATA)) 
                {
                    // read audio flag
                    tb_byte_t data[2];
                    if (!tb_stream_bread(stream, data, 2)) goto end;
                    ...
                    if ((data[0] & TB_FLV_AUDIO_CODEC_MASK) != TB_FLV_AUDIO_CODEC_AAC) 
                    {
                        ...
                        if (data_size > 2)
                        {
                            if (!tb_stream_skip(stream, data_size - 2)) goto end;
                        }
                        break;
                    }
                    ...
                    else if (0x01 == data[1] && (flv->spank_type & TB_FLV_SPANK_TYPE_AUDIO_DATA))
                    {
                        ...
                        if (!tb_stream_bread(stream, flv->audio_data, flv->audio_size)) goto end;
        ...
        case TB_FLV_TAG_TYPE_VIDEO: // video data
            {
                ...
                if (flv->spank_type & (TB_FLV_SPANK_TYPE_VIDEO_CONFIG | TB_FLV_SPANK_TYPE_VIDEO_DATA)) 
                {
                    // read video flag
                    tb_byte_t data[5];
                    if (!tb_stream_bread(stream, data, 5)) goto end;
                    ...
                    if ((data[0] & TB_FLV_VIDEO_CODEC_MASK) != TB_FLV_VIDEO_CODEC_H264) 
                    {
                        ...
                        if (data_size > 5)
                        {
                            if (!tb_stream_skip(stream, data_size - 5)) goto end;
                        }
                        break;
                    }
                    ...
                    else if (1 == type && (flv->spank_type & TB_FLV_SPANK_TYPE_VIDEO_DATA))
                    {
                        ...
                        if (!tb_stream_bread(stream, flv->video_data, flv->video_size)) goto end;
