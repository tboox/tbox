...
static tb_char_t const* tb_xml_reader_text_parse(tb_xml_reader_t* reader)
{
	...
	while (tb_gstream_need(reader->rstream, &pc, 1) && pc)
	{
		...
		else
		{
			...
			if (!tb_gstream_skip(reader->rstream, 1)) return tb_null;
		}
	}
	return tb_null;
}
...
tb_void_t tb_xml_reader_clear(tb_handle_t reader)
{
	...
	// reset stream
	if (xreader->rstream) tb_gstream_seek(xreader->rstream, 0);
			...
			if (size > 4 && !tb_strnicmp(element, "?xml", 4))
			{
				...
				if (tb_pstring_cstricmp(&xreader->charset, "utf-8") && tb_pstring_cstricmp(&xreader->charset, "utf8"))
				{
					...
					if (charset != TB_CHARSET_TYPE_UTF8)
					{
						...
						if (xreader->filter && tb_gstream_open(xreader->filter))
...
tb_bool_t tb_xml_reader_goto(tb_handle_t reader, tb_char_t const* path)
{
	...
	// reset
	xreader->level = 0;
	if (!tb_gstream_seek(xreader->rstream, 0)) return tb_false;
	...
	tb_sstring_exit(&s);
	xreader->level = 0;
	if (!ok) tb_gstream_seek(xreader->rstream, 0);
	return ok;
}
