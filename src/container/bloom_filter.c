...
tb_bloom_filter_ref_t tb_bloom_filter_init(tb_size_t probability, tb_size_t hash_count, tb_size_t item_maxn, tb_item_func_t func)
{
    ...
    do
    {
        ...
        // m = (s * n) >> 16
        tb_size_t m = tb_fixed_mul(s_scale[hash_count - 1][probability], maxn);
#endif
...
{
    // check
    tb_bloom_filter_impl_t* impl = (tb_bloom_filter_impl_t*)bloom_filter;
    tb_assert_and_check_return_val(impl && impl->data && impl->size && stream, tb_false);
    tb_assert_and_check_return_val(impl->func.load && impl->func.free, tb_false);

    // clear the bloom_filter first
    tb_bloom_filter_clear(bloom_filter);
    ...
    {
        // calc type
        crc32 = tb_crc_encode_cstr(TB_CRC_MODE_32_IEEE_LE, crc32, "bloom_filter");

        // calc maxn
        crc32 = tb_crc_encode_value(TB_CRC_MODE_32_IEEE_LE, crc32, impl->maxn);

        // calc size
        crc32 = tb_crc_encode_value(TB_CRC_MODE_32_IEEE_LE, crc32, impl->size);

        // calc mask
        crc32 = tb_crc_encode_value(TB_CRC_MODE_32_IEEE_LE, crc32, impl->mask);

        // calc hash_count
        crc32 = tb_crc_encode_value(TB_CRC_MODE_32_IEEE_LE, crc32, impl->hash_count);

        // calc probability
        crc32 = tb_crc_encode_value(TB_CRC_MODE_32_IEEE_LE, crc32, impl->probability);
        ...
        // load data
        if (!tb_stream_bread(stream, impl->data, impl->size)) break;

        // calc data
        crc32 = tb_crc_encode(TB_CRC_MODE_32_IEEE_LE, crc32, impl->data, impl->size);
...
tb_bool_t tb_bloom_filter_save(tb_bloom_filter_ref_t bloom_filter, tb_stream_ref_t stream)
{
    ...
    do
    {
        ...
        // save data
        if (!tb_stream_bwrit(stream, impl->data, impl->size)) break;
