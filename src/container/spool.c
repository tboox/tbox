...
tb_pointer_t tb_spool_malloc(tb_spool_t* spool, tb_size_t size)
{
	...
	if (spool->pred)
	{
		...
		if (chunk) 
		{
			...
			// try allocating it
			tb_pointer_t p = tb_vpool_malloc(chunk->pool, size);
	...
	if (itor)
	{
		...
		if (chunk) 
		{
			...
			do 
			{
				...
				// init chunk pool
				chunk->pool = tb_vpool_init(chunk->data, chunk->size, 0);
...
tb_pointer_t tb_spool_realloc(tb_spool_t* spool, tb_pointer_t data, tb_size_t size)
{
	...
	if (spool->pred)
	{
		...
		if (chunk) 
		{
			...
			// try allocating it
			tb_pointer_t p = tb_vpool_ralloc(chunk->pool, data, size);
...
tb_void_t tb_spool_free(tb_spool_t* spool, tb_pointer_t data)
{
	...
	if (spool->pred)
	{
		...
		if (chunk) 
		{
			...
			// try freeing it
			if (tb_vpool_free(chunk->pool, data)) return ;
		}
	}
	...
	if (tb_slist_size(spool->list))
	{
		...
		for (; itor != tail; itor = tb_slist_itor_next(spool->list, itor))
		{
			...
			if (chunk) 
			{
				...
				// try free it
				if (tb_vpool_free(chunk->pool, data))
				{
					// update predicted info
					spool->pred = itor;
