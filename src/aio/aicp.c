...
tb_aicp_t* tb_aicp_init(tb_size_t type, tb_size_t maxn)
{
	...
	// alloc aicp
	tb_aicp_t* aicp = tb_calloc(1, sizeof(tb_aicp_t));
	...
	tb_assert_and_check_goto(aicp->pool, end);
...
{
	tb_assert_and_check_return_val(aicp && aicp->rtor->addo, TB_NULL);

	// init
	tb_bool_t r = TB_FALSE;

	// enter 
	if (aicp->mutx) tb_mutex_enter(aicp->mutx);

	// check
	tb_assert_and_check_goto(aicp->rtor && aicp->size < aicp->maxn, end);
	tb_assert_and_check_goto(handle && aicb, end);

	// add object to native
	if (!aicp->rtor->addo(aicp->rtor, handle, aicb, odata)) goto end;
...
tb_bool_t tb_aicp_delo(tb_aicp_t* aicp, tb_handle_t handle, tb_aicb_t aicb, tb_pointer_t odata)
{
	tb_assert_and_check_return_val(aicp && aicp->rtor->addo, TB_FALSE);
	...
	// check
	tb_assert_and_check_goto(aicp->rtor && aicp->size, end);
	tb_assert_and_check_goto(handle && aicb, end);

	// del object from native
	if (!aicp->rtor->delo(aicp->rtor, handle, aicb, odata)) goto end;
