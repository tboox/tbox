...
static tb_size_t tb_aicp_dns_reqt_init(tb_aicp_dns_t* dns)
{
	...
	tb_bits_stream_init(&bst, dns->data, TB_DNS_RPKT_MAXN);
	...
	tb_char_t* p = tb_bits_stream_set_string(&bst, dns->host);

	// only one question now.
	tb_bits_stream_set_u16_be(&bst, 1); 		// we are requesting the ipv4 dnsess
	...
	// ok?
	return tb_bits_stream_offset(&bst);
}
static tb_bool_t tb_aicp_dns_resp_done(tb_aicp_dns_t* dns, tb_size_t size, tb_ipv4_t* ipv4)
{
	...
	tb_bits_stream_init(&bst, dns->data, size);
