...
typedef struct __tb_transfer_stream_open_t
{
	// the func
	tb_transfer_stream_open_func_t 	func;

	// the priv
	tb_pointer_t 			priv;

}tb_transfer_stream_open_t;

// the tstream save type
typedef struct __tb_transfer_stream_save_t
{
	// the func
...
}tb_transfer_stream_save_t;

// the tstream osave type
typedef struct __tb_transfer_stream_osave_t
{
	// the func
	tb_transfer_stream_save_func_t 	func;
...
}tb_transfer_stream_osave_t;

// the tstream type
typedef struct __tb_transfer_stream_t
{
	// the istream
	tb_handle_t 			istream;
	...
	union
	{
		tb_transfer_stream_open_t 	open;
		tb_transfer_stream_save_t 	save;

	} func;

	// open and save
	tb_transfer_stream_osave_t 		osave;

}tb_transfer_stream_t;
 
/* //////////////////////////////////////////////////////////////////////////////////////
 * implementation
 */
static tb_bool_t tb_transfer_stream_istream_read_func(tb_async_stream_t* astream, tb_size_t state, tb_byte_t const* data, tb_size_t real, tb_size_t size, tb_pointer_t priv);
static tb_bool_t tb_transfer_stream_ostream_writ_func(tb_async_stream_t* astream, tb_size_t state, tb_byte_t const* data, tb_size_t real, tb_size_t size, tb_pointer_t priv)
{
	// check
	tb_transfer_stream_t* tstream = (tb_transfer_stream_t*)priv;
	...
	do
	{
		...
		else
		{
			...
			// done func
			if (!tstream->func.save.func(TB_STATE_OK, tb_stream_offset(tstream->istream), tb_stream_size(tstream->istream), tstream->save, tstream->crate, tstream->func.save.priv)) break;
		}
		...
		else if (tb_atomic_get(&tstream->pausing) || tb_atomic_get(&tstream->paused))
		{
			...
			// done func
			if (!tstream->func.save.func(TB_STATE_PAUSED, tb_stream_offset(tstream->istream), tb_stream_size(tstream->istream), tstream->save, 0, tstream->func.save.priv)) break;
		}
		// continue?
		else 
		{
			...
			// continue to read it
			if (!tb_async_stream_read_after(tstream->istream, delay, lrate, tb_transfer_stream_istream_read_func, (tb_pointer_t)tstream)) break;
		}
...
static tb_bool_t tb_transfer_stream_ostream_sync_func(tb_async_stream_t* astream, tb_size_t state, tb_bool_t bclosing, tb_pointer_t priv)
{
	// check
	tb_transfer_stream_t* tstream = (tb_transfer_stream_t*)priv;
	tb_assert_and_check_return_val(astream && tstream && tstream->istream && tstream->func.save.func, tb_false);
	...
	// done func
	return tstream->func.save.func(state == TB_STATE_OK? TB_STATE_CLOSED : state, tb_stream_offset(tstream->istream), tb_stream_size(tstream->istream), tstream->save, trate, tstream->func.save.priv);
}
static tb_bool_t tb_transfer_stream_istream_read_func(tb_async_stream_t* astream, tb_size_t state, tb_byte_t const* data, tb_size_t real, tb_size_t size, tb_pointer_t priv)
{
	// check
	...
	do
	{
		...
		{
			// writ it
			if (!tb_basic_stream_bwrit(tstream->ostream, data, real)) break;
			...
			// done func
			if (!tstream->func.save.func(TB_STATE_OK, tb_stream_offset(astream), tb_stream_size(astream), tstream->save, tstream->crate, tstream->func.save.priv)) break;
			...
			if (tb_atomic_get(&tstream->pausing) || tb_atomic_get(&tstream->paused))
			{
				...
				// done func
				if (!tstream->func.save.func(TB_STATE_PAUSED, tb_stream_offset(astream), tb_stream_size(astream), tstream->save, 0, tstream->func.save.priv)) break;
			}
			// continue?
			else
			{
				...
				else 
				{
					...
					if (!tb_async_stream_read_after(tstream->istream, delay, lrate, tb_transfer_stream_istream_read_func, tstream)) break;
				}
			}
		}
		else 
	...
	if (state != TB_STATE_OK) 
	{
		...
		if (state == TB_STATE_CLOSED)
		{
			if (tb_stream_mode(tstream->ostream) == TB_STREAM_MODE_AIOO)
				tb_basic_stream_sync(tstream->ostream, tb_true);
			else if (tb_stream_mode(tstream->ostream) == TB_STREAM_MODE_AICO)
				bend = tb_async_stream_sync(tstream->ostream, tb_true, tb_transfer_stream_ostream_sync_func, tstream)? tb_false : tb_true;
		}
...
static tb_bool_t tb_transfer_stream_istream_open_func(tb_async_stream_t* astream, tb_size_t state, tb_hize_t offset, tb_pointer_t priv)
{
	// check
	tb_transfer_stream_t* tstream = (tb_transfer_stream_t*)priv;
	...
	do
	{
		...
		// done func
		ok = tstream->func.open.func(TB_STATE_OK, tb_stream_offset(astream), tb_stream_size(astream), tstream->func.open.priv);
...
static tb_bool_t tb_transfer_stream_ostream_open_func(tb_async_stream_t* astream, tb_size_t state, tb_pointer_t priv)
{
	// check
	tb_transfer_stream_t* tstream = (tb_transfer_stream_t*)priv;
	tb_assert_and_check_return_val(astream && tstream && tstream->func.open.func, tb_false);
	...
	do
	{
		...
		if (!tb_async_stream_oseek(tstream->istream, tstream->offset, tb_transfer_stream_istream_open_func, tstream)) break;
...
static tb_bool_t tb_transfer_stream_open_func(tb_size_t state, tb_hize_t offset, tb_hong_t size, tb_pointer_t priv)
{
	// check
	tb_transfer_stream_osave_t* osave = (tb_transfer_stream_osave_t*)priv;
	tb_assert_and_check_return_val(osave && osave->func, tb_false);

	// the tstream
	tb_transfer_stream_t* tstream = (tb_transfer_stream_t*)osave->tstream;
	...
	do
	{
		...
		// save it
		if (!tb_transfer_stream_save(tstream, osave->func, osave->priv)) break;
...
tb_hong_t tb_transfer_stream_save_gu(tb_basic_stream_t* istream, tb_char_t const* ourl, tb_size_t lrate, tb_transfer_stream_save_func_t func, tb_pointer_t priv)
{
	...
	do
	{
		...
		// save stream
		size = tb_transfer_stream_save_gg(istream, ostream, lrate, func, priv);
...
{
	// done
	tb_transfer_stream_t* tstream = tb_null;
	do
	{
		// check
		...
		// make tstream
		tstream = tb_malloc0(sizeof(tb_transfer_stream_t));
...
tb_bool_t tb_transfer_stream_open(tb_handle_t handle, tb_transfer_stream_open_func_t func, tb_pointer_t priv)
{
	// check
	tb_transfer_stream_t* tstream = (tb_transfer_stream_t*)handle;
	...
	do
	{
		...
		// init some rate info
		tstream->base 	= tb_aicp_time(tb_async_stream_aicp(tstream->istream));
		...
		if (tb_stream_mode(tstream->ostream) == TB_STREAM_MODE_AICO)
		{
			...
			{
				if (!tb_async_stream_open(tstream->ostream, tb_transfer_stream_ostream_open_func, tstream)) break;
			}
			else bopened = tb_true;
		}
		else if (tb_stream_mode(tstream->ostream) == TB_STREAM_MODE_AIOO)
		{
			if (!tb_stream_is_opened(tstream->ostream) && !tb_basic_stream_open(tstream->ostream)) break;
			bopened = tb_true;
		}
		else tb_assert_and_check_break(0);

		// open and seek istream
		if (bopened && !tb_async_stream_oseek(tstream->istream, tstream->offset, tb_transfer_stream_istream_open_func, tstream)) break;
...
tb_bool_t tb_transfer_stream_save(tb_handle_t handle, tb_transfer_stream_save_func_t func, tb_pointer_t priv)
{
	// check
	tb_transfer_stream_t* tstream = (tb_transfer_stream_t*)handle;
	tb_assert_and_check_return_val(tstream && func, tb_false);
	...
	// read it
	return tb_async_stream_read(tstream->istream, (tb_size_t)tb_atomic_get(&tstream->lrate), tb_transfer_stream_istream_read_func, tstream);
}
tb_bool_t tb_transfer_stream_osave(tb_handle_t handle, tb_transfer_stream_save_func_t func, tb_pointer_t priv)
{
	// check
	tb_transfer_stream_t* tstream = (tb_transfer_stream_t*)handle;
	tb_assert_and_check_return_val(tstream && func, tb_false);
	...
	if (!tb_atomic_get(&tstream->opened))
	{
		...
		return tb_transfer_stream_open(tstream, tb_transfer_stream_open_func, &tstream->osave);
	}

	// save it
	return tb_transfer_stream_save(tstream, func, priv);
}
tb_void_t tb_transfer_stream_kill(tb_handle_t handle)
{
	// check
	tb_transfer_stream_t* tstream = (tb_transfer_stream_t*)handle;
...
tb_void_t tb_transfer_stream_exit(tb_handle_t handle, tb_bool_t bcalling)
{
	// check
	tb_transfer_stream_t* tstream = (tb_transfer_stream_t*)handle;
	tb_assert_and_check_return(tstream);
	...
	// kill it first 
	tb_transfer_stream_kill(tstream);

	// exit istream
	if (tstream->istream && tstream->iowner) tb_async_stream_exit(tstream->istream, bcalling);
	...
	{
		if (tb_stream_mode(tstream->ostream) == TB_STREAM_MODE_AICO) 
			tb_async_stream_exit(tstream->ostream, bcalling);
		else if (tb_stream_mode(tstream->ostream) == TB_STREAM_MODE_AIOO)
			tb_basic_stream_exit(tstream->ostream);
	}
...
tb_void_t tb_transfer_stream_pause(tb_handle_t handle)
{
	// check
	tb_transfer_stream_t* tstream = (tb_transfer_stream_t*)handle;
	tb_assert_and_check_return(tstream);
...
tb_bool_t tb_transfer_stream_resume(tb_handle_t handle)
{
	// check
	tb_transfer_stream_t* tstream = (tb_transfer_stream_t*)handle;
	...
	do
	{
		...
		// read it
		if (!tb_async_stream_read(tstream->istream, (tb_size_t)tb_atomic_get(&tstream->lrate), tb_transfer_stream_istream_read_func, tstream)) 
		{
...
tb_void_t tb_transfer_stream_limitrate(tb_handle_t handle, tb_size_t rate)
{
	// check
	tb_transfer_stream_t* tstream = (tb_transfer_stream_t*)handle;
	tb_assert_and_check_return(tstream);
...
tb_void_t tb_transfer_stream_timeout_set(tb_handle_t handle, tb_long_t timeout)
{
	// check
	tb_transfer_stream_t* tstream = (tb_transfer_stream_t*)handle;
