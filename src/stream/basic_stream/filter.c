...
static __tb_inline__ tb_basic_stream_filter_t* tb_basic_stream_filter_cast(tb_handle_t stream)
{
	...
	return (tb_basic_stream_filter_t*)gstream;
}
static tb_bool_t tb_basic_stream_filter_open(tb_handle_t gstream)
{
	// check
	tb_basic_stream_filter_t* fstream = tb_basic_stream_filter_cast(gstream);
	...
	// ok
	return tb_basic_stream_open(fstream->gstream);
}
static tb_bool_t tb_basic_stream_filter_clos(tb_handle_t gstream)
{
	// check
	tb_basic_stream_filter_t* fstream = tb_basic_stream_filter_cast(gstream);
	tb_assert_and_check_return_val(fstream && fstream->gstream, tb_false);
	...
	if (fstream->filter && fstream->mode == -1)
	{
		...
		{
			// writ data
			if (!tb_basic_stream_bwrit(fstream->gstream, data, size)) return tb_false;
		}
	}

	// done
	tb_bool_t ok = tb_basic_stream_clos(fstream->gstream);
...
static tb_void_t tb_basic_stream_filter_exit(tb_handle_t gstream)
{	
	// check
	tb_basic_stream_filter_t* fstream = tb_basic_stream_filter_cast(gstream);
...
static tb_void_t tb_basic_stream_filter_kill(tb_handle_t gstream)
{	
	// check
	tb_basic_stream_filter_t* fstream = tb_basic_stream_filter_cast(gstream);
	tb_assert_and_check_return(fstream);
...
static tb_long_t tb_basic_stream_filter_read(tb_handle_t gstream, tb_byte_t* data, tb_size_t size)
{
	// check
	tb_basic_stream_filter_t* fstream = tb_basic_stream_filter_cast(gstream);
	...
	// read 
	tb_long_t real = tb_basic_stream_aread(fstream->gstream, data, size);
...
static tb_long_t tb_basic_stream_filter_writ(tb_handle_t gstream, tb_byte_t const* data, tb_size_t size)
{
	// check
	tb_basic_stream_filter_t* fstream = tb_basic_stream_filter_cast(gstream);
	tb_assert_and_check_return_val(fstream && fstream->gstream, -1);
	...
	// writ 
	return tb_basic_stream_awrit(fstream->gstream, data, size);
}
static tb_bool_t tb_basic_stream_filter_sync(tb_handle_t gstream, tb_bool_t bclosing)
{
	// check
	tb_basic_stream_filter_t* fstream = tb_basic_stream_filter_cast(gstream);
	tb_assert_and_check_return_val(fstream && fstream->gstream, tb_false);
	...
	if (fstream->filter)
	{
		...
		while ( !tb_atomic_get(&fstream->base.base.bstoped)
			...
			&& 	data)
		{
			if (!tb_basic_stream_bwrit(fstream->gstream, data, real)) return tb_false;
		}
	}

	// writ 
	return tb_basic_stream_sync(fstream->gstream, bclosing);
}
static tb_long_t tb_basic_stream_filter_wait(tb_handle_t gstream, tb_size_t wait, tb_long_t timeout)
{
	// check
	tb_basic_stream_filter_t* fstream = tb_basic_stream_filter_cast(gstream);
	tb_assert_and_check_return_val(fstream && fstream->gstream, -1);
	...
	if (fstream->filter && fstream->mode == 1)
	{
		...
		{
			// wait
			ok = tb_basic_stream_wait(fstream->gstream, wait, timeout);
	...
	else ok = tb_basic_stream_wait(fstream->gstream, wait, timeout);

	// ok?
	return ok;
}
static tb_bool_t tb_basic_stream_filter_ctrl(tb_handle_t gstream, tb_size_t ctrl, tb_va_list_t args)
{
	// check
	tb_basic_stream_filter_t* fstream = tb_basic_stream_filter_cast(gstream);
	...
	case TB_STREAM_CTRL_FLTR_SET_STREAM:
		{
			...
			// set gstream
			tb_basic_stream_t* gstream = (tb_basic_stream_t*)tb_va_arg(args, tb_basic_stream_t*);
	...
	case TB_STREAM_CTRL_FLTR_GET_STREAM:
		{
			// get gstream
			tb_basic_stream_t** pgstream = (tb_basic_stream_t**)tb_va_arg(args, tb_basic_stream_t**);
...
{
	// make stream
	tb_basic_stream_filter_t* gstream = (tb_basic_stream_filter_t*)tb_malloc0(sizeof(tb_basic_stream_filter_t));
	tb_assert_and_check_return_val(gstream, tb_null);

	// init base
	if (!tb_basic_stream_init((tb_basic_stream_t*)gstream, TB_STREAM_TYPE_FLTR, 0)) goto fail;

	// init stream
	gstream->base.open		= tb_basic_stream_filter_open;
	gstream->base.clos 		= tb_basic_stream_filter_clos;
	gstream->base.exit 		= tb_basic_stream_filter_exit;
	gstream->base.read 		= tb_basic_stream_filter_read;
	gstream->base.writ 		= tb_basic_stream_filter_writ;
	gstream->base.sync 		= tb_basic_stream_filter_sync;
	gstream->base.wait 		= tb_basic_stream_filter_wait;
	gstream->base.base.ctrl = tb_basic_stream_filter_ctrl;
	gstream->base.base.kill = tb_basic_stream_filter_kill;
