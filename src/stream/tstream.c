...
typedef struct __tb_tstream_t
{
	...
	// the save func
	tb_tstream_save_func_t 	func_save;

	// the open func
	tb_tstream_open_func_t 	func_open;

	// the func priv
	tb_pointer_t 			func_priv;
...
static tb_bool_t tb_tstream_ostream_writ_func(tb_astream_t* astream, tb_size_t state, tb_byte_t const* data, tb_size_t real, tb_size_t size, tb_pointer_t priv)
{
	...
	tb_assert_and_check_return_val(astream && tstream && tstream->func_save, tb_false);
...
static tb_bool_t tb_tstream_ostream_sync_func(tb_astream_t* astream, tb_size_t state, tb_pointer_t priv)
{
	...
	// done func
	return tstream->func_save(tstream, state == TB_STREAM_STATE_OK? TB_STREAM_STATE_CLOSED : state, tstream->save, trate, tstream->func_priv);
}
static tb_bool_t tb_tstream_istream_read_func(tb_astream_t* astream, tb_size_t state, tb_byte_t const* data, tb_size_t real, tb_size_t size, tb_pointer_t priv)
{
	// check
	...
	do
	{
		...
		else if (tstream->type == TB_TSTREAM_TYPE_AG)
		{
			...
			// done func
			if (!tstream->func_save(tstream, TB_STREAM_STATE_OK, tstream->save, tstream->crate, tstream->func_priv)) break;
...
static tb_bool_t tb_tstream_ostream_open_func(tb_astream_t* astream, tb_size_t state, tb_pointer_t priv)
{
	...
	// trace
	tb_trace_impl("open: state: %s", tb_stream_state_cstr(state));
	...
	do
	{
		...
		{
			// open and seek it
			if (!tb_astream_oseek(tstream->istream, tstream->seek, tb_tstream_istream_seek_func, tstream)) break;
		}
		else
		{
			// open and read it
...
tb_hong_t tb_tstream_save_gg(tb_gstream_t* istream, tb_gstream_t* ostream, tb_size_t lrate, tb_tstream_save_func_t func, tb_pointer_t priv)
{
		...
		if (real > 0)
		{
			...
			if (func || lrate) 
			{
				...
				else
				{
					...
					// done func
					if (func) func(&tstream, TB_STREAM_STATE_OK, writ, crate, priv);
				}
	...
	if (func) 
	{
		...
		// done func
		func(&tstream, TB_STREAM_STATE_CLOSED, writ, trate, priv);
	}
