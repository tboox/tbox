...
tb_gstream_t* tb_gstream_init_from_url(tb_char_t const* url)
{
	...
	// set url
	if (!tb_gstream_ctrl(gstream, TB_GSTREAM_CTRL_SET_URL, url)) goto fail;
...
tb_long_t tb_gstream_wait(tb_gstream_t* gstream, tb_size_t wait, tb_long_t timeout)
{
	...
	// wait failed
	if (ok < 0 && !gstream->state) gstream->state = TB_GSTREAM_STATE_WAIT_FAILED;
...
{
	// check
	tb_assert_and_check_return_val(gstream, TB_GSTREAM_STATE_UNKNOWN_ERROR);
...
{
	tb_assert_and_check_return_val(gstream, TB_GSTREAM_TYPE_NONE);
	return gstream->type;
}
tb_hize_t tb_gstream_size(tb_gstream_t const* gstream)
{
	tb_assert_and_check_return_val(gstream, 0);
	return gstream->size? gstream->size((tb_gstream_t*)gstream) : 0;
}
...
{	
	...
	case TB_GSTREAM_CTRL_SET_URL:
		{
			...
			if (url && tb_url_set(&gstream->url, url)) ret = tb_true;
		}
		break;
	...
	case TB_GSTREAM_CTRL_SET_CACHE:
		{
			...
			if (cache)
			{
				...
				if (tb_qbuffer_maxn(&gstream->cache)) ret = tb_true;
				gstream->bcached = 1;
			}
	...
	ret = (gstream->ctrl(gstream, ctrl, args) || ret)? tb_true : tb_false;
