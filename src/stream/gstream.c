...
static tb_long_t tb_gstream_cache_wait(tb_gstream_t* gst, tb_size_t wait, tb_long_t timeout)
{
	tb_assert_and_check_return_val(gst, -1);

	// wait the native event first
	tb_long_t e = gst->wait(gst, wait, timeout);
	...
	// wait for cache
	if ((wait & TB_GSTREAM_WAIT_READ) && !tb_qbuffer_null(&gst->cache)) e |= TB_GSTREAM_WAIT_READ;
	if ((wait & TB_GSTREAM_WAIT_WRIT) && !tb_qbuffer_full(&gst->cache)) e |= TB_GSTREAM_WAIT_WRIT;
...
tb_long_t tb_gstream_wait(tb_gstream_t* gst, tb_size_t wait, tb_long_t timeout)
{
	...
	// wait
	tb_long_t ok = tb_gstream_cache_wait(gst, wait, timeout);
...
tb_char_t const* tb_gstream_state_cstr(tb_gstream_t* gst)
{
	...
	case TB_SSTREAM_STATE_DNS_FAILED: 		return "sock: dns: failed";
	case TB_SSTREAM_STATE_CONNECT_FAILED: 	return "sock: connect: failed";
	case TB_SSTREAM_STATE_SSL_FAILED: 		return "sock: ssl: failed";
	case TB_HSTREAM_STATE_RESPONSE_204: 	return "http: response: 204";
	case TB_HSTREAM_STATE_RESPONSE_300: 	return "http: response: 300";
	case TB_HSTREAM_STATE_RESPONSE_301: 	return "http: response: 301";
	case TB_HSTREAM_STATE_RESPONSE_302: 	return "http: response: 302";
	case TB_HSTREAM_STATE_RESPONSE_303: 	return "http: response: 303";
	case TB_HSTREAM_STATE_RESPONSE_304: 	return "http: http: response: 304";
	case TB_HSTREAM_STATE_RESPONSE_400: 	return "http: response: 400";
	case TB_HSTREAM_STATE_RESPONSE_401: 	return "http: response: 401";
	case TB_HSTREAM_STATE_RESPONSE_402: 	return "http: response: 402";
	case TB_HSTREAM_STATE_RESPONSE_403: 	return "http: response: 403";
	case TB_HSTREAM_STATE_RESPONSE_404: 	return "http: response: 404";
	case TB_HSTREAM_STATE_RESPONSE_405: 	return "http: response: 405";
	case TB_HSTREAM_STATE_RESPONSE_406: 	return "http: response: 406";
	case TB_HSTREAM_STATE_RESPONSE_407: 	return "http: response: 407";
	case TB_HSTREAM_STATE_RESPONSE_408: 	return "http: response: 408";
	case TB_HSTREAM_STATE_RESPONSE_409: 	return "http: response: 409";
	case TB_HSTREAM_STATE_RESPONSE_410: 	return "http: response: 410";
	case TB_HSTREAM_STATE_RESPONSE_411: 	return "http: response: 411";
	case TB_HSTREAM_STATE_RESPONSE_412: 	return "http: response: 412";
	case TB_HSTREAM_STATE_RESPONSE_413: 	return "http: response: 413";
	case TB_HSTREAM_STATE_RESPONSE_414: 	return "http: response: 414";
	case TB_HSTREAM_STATE_RESPONSE_415: 	return "http: response: 415";
	case TB_HSTREAM_STATE_RESPONSE_416: 	return "http: response: 416";
	case TB_HSTREAM_STATE_RESPONSE_500: 	return "http: response: 500";
	case TB_HSTREAM_STATE_RESPONSE_501: 	return "http: response: 501";
	case TB_HSTREAM_STATE_RESPONSE_502: 	return "http: response: 502";
	case TB_HSTREAM_STATE_RESPONSE_503: 	return "http: response: 503";
	case TB_HSTREAM_STATE_RESPONSE_504: 	return "http: response: 504";
	case TB_HSTREAM_STATE_RESPONSE_505: 	return "http: response: 505";
	case TB_HSTREAM_STATE_RESPONSE_506: 	return "http: response: 506";
	case TB_HSTREAM_STATE_RESPONSE_507: 	return "http: response: 507";
	case TB_HSTREAM_STATE_RESPONSE_UNK: 	return "http: response: unknown code";
	case TB_HSTREAM_STATE_RESPONSE_NUL: 	return "http: response: no";
	case TB_HSTREAM_STATE_REQUEST_FAILED: 	return "http: request: failed";
	case TB_HSTREAM_STATE_UNKNOWN_ERROR: 	return "http: unknown error";
...
tb_bool_t tb_gstream_bopen(tb_gstream_t* gst)
{
	...
	{
		// wait
		r = tb_gstream_wait(gst, TB_GSTREAM_WAIT_EALL, gst->timeout);
...
tb_bool_t tb_gstream_bneed(tb_gstream_t* gst, tb_byte_t** data, tb_size_t size)
{
	...
	while ((need = tb_gstream_aneed(gst, data, size)) < size)
	{
		...
		{
			// wait
			tb_long_t e = tb_gstream_wait(gst, TB_GSTREAM_WAIT_READ, gst->timeout);
...
tb_bool_t tb_gstream_bread(tb_gstream_t* gst, tb_byte_t* data, tb_size_t size)
{
	...
	// check the left
	tb_hize_t left = tb_gstream_left(gst);
	...
	{
		// read data
		tb_long_t real = tb_gstream_aread(gst, data + read, size - read);	
		if (real > 0) read += real;
		else if (!real)
		{
			// wait
...
tb_bool_t tb_gstream_bwrit(tb_gstream_t* gst, tb_byte_t* data, tb_size_t size)
{
	...
	{
		// writ data
		tb_long_t real = tb_gstream_awrit(gst, data + writ, size - writ);	
		if (real > 0) writ += real;
		else if (!real)
		{
			// wait
			tb_long_t e = tb_gstream_wait(gst, TB_GSTREAM_WAIT_WRIT, gst->timeout);
			tb_assert_and_check_break(e >= 0);
...
tb_bool_t tb_gstream_bfread(tb_gstream_t* gst, tb_byte_t* data, tb_size_t size)
{
	...
	if (data && size)
	{
		...
		{
			// read data
			tb_long_t real = tb_gstream_afread(gst, data + read, size - read);	
			// has data?
			if (real > 0) read += real;
			// no data?
			else if (!real)
...
tb_bool_t tb_gstream_bfwrit(tb_gstream_t* gst, tb_byte_t* data, tb_size_t size)
{
	...
	if (data && size)
	{
		...
		{
			// writ data
			tb_long_t real = tb_gstream_afwrit(gst, data + writ, size - writ);	
...
tb_long_t tb_gstream_bread_line(tb_gstream_t* gst, tb_char_t* data, tb_size_t size)
{
	...
	// end?
	return tb_gstream_beof(gst)? -1 : 0;
}
...
tb_hize_t tb_gstream_load(tb_gstream_t* gst, tb_gstream_t* ist)
{
	...
	do
	{
		...
		{
			// writ data
			if (!tb_gstream_bwrit(gst, data, real)) break;
		...
		{
			// wait
			tb_long_t e = tb_gstream_wait(ist, TB_GSTREAM_WAIT_READ, ist->timeout);
			tb_assert_and_check_break(e >= 0);
