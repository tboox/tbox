...
tb_long_t tb_gstream_wait(tb_gstream_t* gstream, tb_size_t wait, tb_long_t timeout)
{
	...
	{
		// have read cache?
		if ((wait & TB_GSTREAM_WAIT_READ) && !gstream->bwrited && !tb_queue_buffer_null(&gstream->cache)) 
			ok |= TB_GSTREAM_WAIT_READ;
		// have writ cache?
		else if ((wait & TB_GSTREAM_WAIT_WRIT) && gstream->bwrited && !tb_queue_buffer_full(&gstream->cache))
...
tb_bool_t tb_gstream_need(tb_gstream_t* gstream, tb_byte_t** data, tb_size_t size)
{
	...
	if (gstream->bwrited && !tb_queue_buffer_null(&gstream->cache) && !tb_gstream_sync(gstream, tb_false)) return tb_false;
...
tb_bool_t tb_gstream_bread(tb_gstream_t* gstream, tb_byte_t* data, tb_size_t size)
{
	...
	// have writed cache? sync first
	if (gstream->bwrited && !tb_queue_buffer_null(&gstream->cache) && !tb_gstream_sync(gstream, tb_false))
		return tb_false;
