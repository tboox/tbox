...
tb_long_t tb_gstream_wait(tb_gstream_t* gstream, tb_size_t wait, tb_long_t timeout)
{
	...
	// cached?
	if (tb_qbuffer_maxn(&gstream->cache))
	{
		// have read cache?
...
tb_bool_t tb_gstream_need(tb_gstream_t* gstream, tb_byte_t** data, tb_size_t size)
{
	...
	// must enable cache
	tb_assert_and_check_return_val(tb_qbuffer_maxn(&gstream->cache) && size <= tb_qbuffer_maxn(&gstream->cache), tb_false);

	// have writed cache? sync first
	if (gstream->bwrited && !tb_qbuffer_null(&gstream->cache) && !tb_gstream_sync(gstream, tb_false)) return tb_false;
...
tb_bool_t tb_gstream_bread(tb_gstream_t* gstream, tb_byte_t* data, tb_size_t size)
{
	...
	// have writed cache? sync first
	if (gstream->bwrited && !tb_qbuffer_null(&gstream->cache) && !tb_gstream_sync(gstream, tb_false))
		return tb_false;
...
tb_bool_t tb_gstream_seek(tb_gstream_t* gstream, tb_hize_t offset)
{
	...
	if (gstream->bwrited)
	{
		...
		tb_assert_and_check_return_val(!tb_qbuffer_maxn(&gstream->cache) || tb_qbuffer_null(&gstream->cache), tb_false);
