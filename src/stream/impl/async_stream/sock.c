...
static tb_void_t tb_async_stream_sock_impl_clos_dns_func(tb_aicp_dns_ref_t dns, tb_cpointer_t priv)
{
    ...
    // trace
    tb_trace_d("clos: dns: notify: %s: ..", tb_url_get(tb_async_stream_url((tb_async_stream_ref_t)impl)));
...
static tb_void_t tb_async_stream_sock_impl_clos_aico_func(tb_aico_ref_t aico, tb_cpointer_t priv)
{
    ...
    // exit dns 
    if (impl->hdns) tb_aicp_dns_exit(impl->hdns, tb_async_stream_sock_impl_clos_dns_func, impl);
    // done func directly
...
#endif
static tb_bool_t tb_async_stream_sock_impl_conn_func(tb_aice_t const* aice)
{
    // check
...
#ifdef TB_SSL_ENABLE
            // ssl?
            if (tb_url_ssl_get(tb_async_stream_url((tb_async_stream_ref_t)impl)))
            {
                // open ssl
...
static tb_void_t tb_async_stream_sock_impl_dns_func(tb_aicp_dns_ref_t dns, tb_char_t const* host, tb_ipv4_t const* addr, tb_cpointer_t priv)
{
    ...
    do
    {
        ...
        if (addr)
        {
            ...
            // the url
            tb_url_t* url = tb_async_stream_url((tb_async_stream_ref_t)impl);
            ...
            {
                // done conn
                if (!tb_aico_conn(impl->aico, addr, port, tb_async_stream_sock_impl_conn_func, impl)) break;
            }
            // udp?
            else
            {
...
static tb_bool_t tb_async_stream_sock_impl_open(tb_async_stream_ref_t stream, tb_async_stream_open_func_t func, tb_cpointer_t priv)
{
    ...
    do
    {
        ...
        // the url
        tb_url_t* url = tb_async_stream_url(stream);
        tb_assert_and_check_break(url);
        ...
        tb_char_t const* host = tb_url_host_get(url);
...
static tb_bool_t tb_async_stream_sock_impl_read_func(tb_aice_t const* aice)
{
    // check
...
static tb_bool_t tb_async_stream_sock_impl_read_udp_func(tb_aice_t const* aice)
{
    // check
    ...
    if (impl->func.read((tb_async_stream_ref_t)impl, state, aice->u.urecv.data, aice->u.urecv.real, aice->u.urecv.size, impl->priv))
    {
        ...
        {
            // continue to post read
            if (!tb_aico_urecv(aice->aico, &aice->u.urecv.addr, aice->u.urecv.port, aice->u.urecv.data, aice->u.urecv.size, tb_async_stream_sock_impl_read_udp_func, (tb_async_stream_ref_t)impl))
...
static tb_bool_t tb_async_stream_sock_impl_read(tb_async_stream_ref_t stream, tb_size_t delay, tb_byte_t* data, tb_size_t size, tb_async_stream_read_func_t func, tb_cpointer_t priv)
{
    ...
    case TB_SOCKET_TYPE_UDP:
        {
            // post udp read
            ok = tb_aico_urecv_after(impl->aico, delay, &impl->ipv4, tb_url_port_get(tb_async_stream_url((tb_async_stream_ref_t)impl)), data, size, tb_async_stream_sock_impl_read_udp_func, stream);
        }
        break;
    default:
...
static tb_bool_t tb_async_stream_sock_impl_writ_func(tb_aice_t const* aice)
{
    // check
...
static tb_bool_t tb_async_stream_sock_impl_writ_udp_func(tb_aice_t const* aice)
{
    // check
    ...
    if (impl->func.writ((tb_async_stream_ref_t)impl, state, aice->u.usend.data, aice->u.usend.real, aice->u.usend.size, impl->priv))
    {
        ...
        {
            // continue to post writ
            if (!tb_aico_usend(aice->aico, &aice->u.usend.addr, aice->u.usend.port, aice->u.usend.data + aice->u.usend.real, aice->u.usend.size - aice->u.usend.real, tb_async_stream_sock_impl_writ_udp_func, (tb_async_stream_ref_t)impl))
                impl->func.writ((tb_async_stream_ref_t)impl, TB_STATE_SOCK_SEND_FAILED, aice->u.usend.data, 0, aice->u.usend.size, impl->priv);
        }
    }
...
static tb_bool_t tb_async_stream_sock_impl_writ(tb_async_stream_ref_t stream, tb_size_t delay, tb_byte_t const* data, tb_size_t size, tb_async_stream_writ_func_t func, tb_cpointer_t priv)
{
    ...
    case TB_SOCKET_TYPE_UDP:
        {
            // post udp writ
            ok = tb_aico_usend_after(impl->aico, delay, &impl->ipv4, tb_url_port_get(tb_async_stream_url((tb_async_stream_ref_t)impl)), data, size, tb_async_stream_sock_impl_writ_udp_func, stream);
        }
        break;
    default:
        tb_trace_e("unknown socket type: %lu", impl->type);
...
static tb_bool_t tb_async_stream_sock_impl_task_func(tb_aice_t const* aice)
{
    // check
...
static tb_bool_t tb_async_stream_sock_impl_task_ssl_func(tb_aicp_ssl_ref_t ssl, tb_size_t state, tb_size_t delay, tb_cpointer_t priv)
{
    ...
    // done func
    tb_bool_t ok = impl->func.task((tb_async_stream_ref_t)impl, state, impl->priv);

    // ok and continue?
    if (ok && state == TB_STATE_OK)
    {
        // post task
        tb_aicp_ssl_task(ssl, delay, tb_async_stream_sock_impl_task_ssl_func, impl);
    }
...
static tb_bool_t tb_async_stream_sock_impl_task(tb_async_stream_ref_t stream, tb_size_t delay, tb_async_stream_task_func_t func, tb_cpointer_t priv)
{
    ...
    // post task
    if (impl->aico) return tb_aico_task_run(impl->aico, delay, tb_async_stream_sock_impl_task_func, stream);
#ifdef TB_SSL_ENABLE
    else if (impl->hssl) return tb_aicp_ssl_task(impl->hssl, delay, tb_async_stream_sock_impl_task_ssl_func, stream);
#endif
