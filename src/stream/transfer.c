...
typedef struct __tb_transfer_transfer_t
{
	// the tstream
	tb_handle_t 			tstream;
 ...
 * implementation
 */
static tb_void_t tb_transfer_transfer_exit(tb_transfer_transfer_t* transfer, tb_bool_t bcalling)
{
	// check
	tb_assert_and_check_return(transfer);

	// the mstream
	tb_transfer_t* mstream = (tb_transfer_t*)transfer->mstream;
...
static tb_bool_t tb_transfer_transfer_save(tb_size_t state, tb_hize_t offset, tb_hong_t size, tb_hize_t save, tb_size_t rate, tb_pointer_t priv)
{
	// the transfer
	tb_transfer_transfer_t* transfer = (tb_transfer_transfer_t*)priv;
	...
	if (state != TB_STATE_OK && state != TB_STATE_PAUSED)
	{
		...
		do
		{
			...
			// exit transfer
			tb_transfer_transfer_exit(transfer, tb_true);
			transfer = tb_null;
		...
		{
			// done
			if (!tb_transfer_stream_osave(transfer->tstream, tb_transfer_transfer_save, transfer))
			{
				// enter
				tb_spinlock_enter(&mstream->lock);
				...
				// exit transfer
				tb_transfer_transfer_exit(transfer, tb_false);
...
static tb_bool_t tb_transfer_transfer_working_kill(tb_iterator_t* iterator, tb_pointer_t item, tb_pointer_t priv)
{
	// the transfer 
	tb_transfer_transfer_t* transfer = (tb_transfer_transfer_t*)item;
...
static tb_bool_t tb_transfer_transfer_working_copy(tb_iterator_t* iterator, tb_pointer_t item, tb_pointer_t priv)
{
	// the transfer 
	tb_transfer_transfer_t* transfer = (tb_transfer_transfer_t*)item;
...
static tb_bool_t tb_transfer_transfer_waiting_exit(tb_iterator_t* iterator, tb_pointer_t item, tb_pointer_t priv)
{
	// the transfer 
	tb_transfer_transfer_t* transfer = (tb_transfer_transfer_t*)item;
	tb_check_return_val(transfer, tb_false);
...
static tb_pointer_t tb_transfer_loop(tb_pointer_t data)
{
	// aicp
...
tb_handle_t tb_transfer_init(tb_aicp_t* aicp, tb_size_t conc, tb_long_t timeout)
{
	...
	tb_transfer_t* 	mstream = tb_null;
	do
	{
		// make mstream
		mstream = (tb_handle_t)tb_malloc0(sizeof(tb_transfer_t));
		...
		// init pool
		mstream->pool = tb_fixed_pool_init(conc? conc : 16, sizeof(tb_transfer_transfer_t), 0);
	...
	{
		// exit it
		if (mstream) tb_transfer_exit((tb_handle_t)mstream);
		mstream = tb_null;
	}
...
{
	// check
	tb_transfer_t* mstream = (tb_transfer_t*)handle;
	...
	if (!mstream->bstoped && mstream->working && tb_dlist_size(mstream->working))
	{
		...
		// copy it
		tb_walk_all(mstream->working, tb_transfer_transfer_working_copy, working);
	}
	...
	if (working)
	{
		tb_walk_all(working, tb_transfer_transfer_working_kill, tb_null);
		tb_dlist_exit(working);
		working = tb_null;
	}
}
tb_void_t tb_transfer_exit(tb_handle_t handle)
{
	// check
	tb_transfer_t* mstream = (tb_transfer_t*)handle;
	tb_assert_and_check_return(mstream);
	...
	{
		// exit it
		tb_walk_all(mstream->waiting, tb_transfer_transfer_waiting_exit, tb_null);
...
tb_size_t tb_transfer_size(tb_handle_t handle)
{
	// check
	tb_transfer_t* mstream = (tb_transfer_t*)handle;
...
tb_bool_t tb_transfer_done(tb_handle_t handle, tb_char_t const* iurl, tb_char_t const* ourl, tb_hize_t offset, tb_transfer_stream_save_func_t func, tb_pointer_t priv)
{
	// check
	tb_transfer_t* mstream = (tb_transfer_t*)handle;
	...
	tb_transfer_transfer_t* 	transfer = tb_null;
	do
	{
		// stoped?
		...
		if (!mstream->loop)
		{
			mstream->loop = tb_thread_init(tb_null, tb_transfer_loop, mstream->aicp, 0);
		...
		transfer->tstream 	= tb_transfer_stream_init_uu(mstream->aicp, iurl, ourl, offset);
		transfer->func 		= func;
		transfer->priv 		= priv;
		tb_assert_and_check_break(transfer->tstream);

		// init timeout
		if (mstream->timeout) tb_transfer_stream_timeout_set(transfer->tstream, mstream->timeout);
	...
	if (!ok && transfer) tb_transfer_transfer_exit(transfer, tb_false);
