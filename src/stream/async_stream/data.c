...
static tb_void_t tb_async_stream_data_clos(tb_handle_t astream, tb_bool_t bcalling)
{	
	// check
	tb_async_stream_data_t* dstream = tb_async_stream_data_cast(astream);
	tb_assert_and_check_return(dstream);
...
static tb_void_t tb_async_stream_data_exit(tb_handle_t astream, tb_bool_t bcalling)
{	
	// check
	tb_async_stream_data_t* dstream = tb_async_stream_data_cast(astream);
	tb_assert_and_check_return(dstream);
...
tb_async_stream_t* tb_async_stream_init_data(tb_aicp_t* aicp)
{
	...
	// make stream
	tb_async_stream_data_t* dstream = (tb_async_stream_data_t*)tb_malloc0(sizeof(tb_async_stream_data_t));
	tb_assert_and_check_return_val(dstream, tb_null);

	// init stream
	if (!tb_async_stream_init((tb_async_stream_t*)dstream, aicp, TB_STREAM_TYPE_DATA, 0, 0)) goto fail;
...
fail:
	if (dstream) tb_async_stream_exit((tb_async_stream_t*)dstream, tb_false);
	return tb_null;
}
tb_async_stream_t* tb_async_stream_init_from_data(tb_aicp_t* aicp, tb_byte_t const* data, tb_size_t size)
{
	...
	// init data stream
	tb_async_stream_t* dstream = tb_async_stream_init_data(aicp);
	tb_assert_and_check_return_val(dstream, tb_null);
	...
	return dstream;
fail:
	if (dstream) tb_async_stream_exit(dstream, tb_false);
	return tb_null;
}
