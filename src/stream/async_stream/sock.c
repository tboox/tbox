...
static __tb_inline__ tb_async_stream_sock_t* tb_async_stream_sock_cast(tb_handle_t stream)
{
    ...
    tb_assert_and_check_return_val(astream && astream->base.type == TB_STREAM_TYPE_SOCK, tb_object_null);
...
static tb_void_t tb_async_stream_sock_clos_dns_func(tb_handle_t aico, tb_cpointer_t priv)
{
    ...
    // clear dns
    sstream->hdns = tb_object_null;

    // done func directly
    tb_async_stream_sock_clos_func(tb_object_null, sstream);
...
static tb_void_t tb_async_stream_sock_clos_aico_func(tb_handle_t aico, tb_cpointer_t priv)
{
    ...
    // clear aico
    sstream->aico = tb_object_null;
#ifdef TB_SSL_ENABLE
    if (sstream->hssl) tb_aicp_ssl_set_aico(sstream->hssl, tb_object_null);
#endif

    // exit dns 
    if (sstream->hdns) tb_aicp_dns_exit(sstream->hdns, tb_async_stream_sock_clos_dns_func, sstream);
    // done func directly
    else tb_async_stream_sock_clos_func(tb_object_null, sstream);
...
#endif
static tb_bool_t tb_async_stream_sock_clos_try(tb_handle_t astream)
{   
    // check
...
static tb_bool_t tb_async_stream_sock_read(tb_handle_t astream, tb_size_t delay, tb_byte_t* data, tb_size_t size, tb_async_stream_read_func_t func, tb_cpointer_t priv)
{
    ...
    case TB_SOCKET_TYPE_TCP:
        {
            ...
            if (tb_url_ssl_get(&sstream->base.base.url))
            {
                ...
                // post ssl read
                ok = tb_aicp_ssl_read_after(sstream->hssl, delay, data, size, tb_async_stream_sock_read_ssl_func, astream);
            }
            // post tcp read
            else
#endif
    ...
    case TB_SOCKET_TYPE_UDP:
        {
            // post udp read
            ok = tb_aico_urecv_after(sstream->aico, delay, &sstream->ipv4, tb_url_port_get(&sstream->base.base.url), data, size, tb_async_stream_sock_read_udp_func, astream);
        }
        break;
    default:
...
static tb_bool_t tb_async_stream_sock_writ(tb_handle_t astream, tb_size_t delay, tb_byte_t const* data, tb_size_t size, tb_async_stream_writ_func_t func, tb_cpointer_t priv)
{
    ...
    case TB_SOCKET_TYPE_TCP:
        {
            ...
            if (tb_url_ssl_get(&sstream->base.base.url))
            {
                ...
                // post ssl writ
                ok = tb_aicp_ssl_writ_after(sstream->hssl, delay, data, size, tb_async_stream_sock_writ_ssl_func, astream);
            }
            // post tcp writ
            else 
#endif
    ...
    case TB_SOCKET_TYPE_UDP:
        {
            // post udp writ
            ok = tb_aico_usend_after(sstream->aico, delay, &sstream->ipv4, tb_url_port_get(&sstream->base.base.url), data, size, tb_async_stream_sock_writ_udp_func, astream);
        }
        break;
    default:
        tb_trace_e("unknown socket type: %lu", sstream->type);
...
#ifdef TB_SSL_ENABLE
    else if (sstream->hssl) return tb_aicp_ssl_task(sstream->hssl, delay, tb_async_stream_sock_task_ssl_func, astream);
#endif
...
{   
    ...
    {
        // exit aico
        if (sstream->aico) tb_aico_exit(sstream->aico, tb_object_null, tb_object_null);
        sstream->aico = tb_object_null;

        // exit hdns
        if (sstream->hdns) tb_aicp_dns_exit(sstream->hdns, tb_object_null, tb_object_null);
        sstream->hdns = tb_object_null;
    }
    else
    {
...
tb_async_stream_t* tb_async_stream_init_sock(tb_aicp_t* aicp)
{
    // done
    tb_bool_t               ok = tb_false;
    tb_async_stream_sock_t* sstream = tb_object_null;
    do
    {
        // make stream
...
tb_async_stream_t* tb_async_stream_init_from_sock(tb_aicp_t* aicp, tb_char_t const* host, tb_size_t port, tb_size_t type, tb_bool_t bssl)
{
    ...
    tb_assert_and_check_return_val(!bssl, tb_object_null);

    // done
    tb_bool_t           ok = tb_false;
    tb_async_stream_t*  sstream = tb_object_null;
    do
    {
        // init stream
        sstream = tb_async_stream_init_sock(aicp);
        tb_assert_and_check_return_val(sstream, tb_object_null);
