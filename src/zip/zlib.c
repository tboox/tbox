...
static void tb_zip_zlib_close(tb_zip_t* zip)
{
...
{
	tb_zip_zlib_t* zlib = tb_zip_zlib_cast(zip);
	TB_ASSERT_RETURN_VAL(zlib && ist && ost, TB_ZIP_STATUS_FAIL);
	...
	//TB_DBG("deflate: %d", zlib->zst.total_out);
...
static tb_zip_status_t tb_zip_zlib_spank_inflate(tb_zip_t* zip, tb_bstream_t* ist, tb_bstream_t* ost)
{
	...
	TB_ASSERT_RETURN_VAL(ret == Z_OK || ret == Z_STREAM_END, TB_ZIP_STATUS_FAIL);
	//TB_DBG("inflate: %d", zlib->zst.total_out);
