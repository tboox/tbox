 ...
 * trace
 */
#define TB_TRACE_IMPL_TAG 	"zlibraw"
...
static tb_long_t tb_zip_zlibraw_spak_deflate(tb_zip_t* zip, tb_bstream_t* ist, tb_bstream_t* ost, tb_long_t sync)
{
	...
	tb_trace_impl("deflate: %u => %u, sync: %ld", ie - ip, (tb_byte_t*)zlibraw->zst.next_out - op, sync);
...
static tb_long_t tb_zip_zlibraw_spak_inflate(tb_zip_t* zip, tb_bstream_t* ist, tb_bstream_t* ost, tb_long_t sync)
{
	tb_zip_zlibraw_t* zlibraw = tb_zip_zlibraw_cast(zip);
	tb_assert_and_check_return_val(zlibraw && ist && ost, -1);
	...
	tb_assert_and_check_return_val(r == Z_OK || r == Z_STREAM_END, -1);
	tb_trace_impl("inflate: %u => %u, sync: %ld", ie - ip, (tb_byte_t*)zlibraw->zst.next_out - op, sync);
...
{	
	// alloc
	tb_zip_t* zip = (tb_zip_t*)tb_malloc0(sizeof(tb_zip_zlibraw_t));
