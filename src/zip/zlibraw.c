...
static tb_long_t tb_zip_zlibraw_spak_deflate(tb_zip_t* zip, tb_bstream_t* ist, tb_bstream_t* ost)
{
	...
	// deflate 
	tb_int_t r = deflate(&zlibraw->zst, Z_NO_FLUSH);
...
static tb_long_t tb_zip_zlibraw_spak_inflate(tb_zip_t* zip, tb_bstream_t* ist, tb_bstream_t* ost)
{
	tb_zip_zlibraw_t* zlibraw = tb_zip_zlibraw_cast(zip);
	tb_assert_and_check_return_val(zlibraw && ist && ost, -1);
	...
	// inflate 
	tb_int_t r = inflate(&zlibraw->zst, Z_NO_FLUSH);
	tb_assert_and_check_return_val(r == Z_OK || r == Z_STREAM_END, -1);
...
{	
	// alloc
	tb_zip_t* zip = (tb_zip_t*)tb_calloc(1, sizeof(tb_zip_zlibraw_t));
