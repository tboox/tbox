...
static tb_long_t tb_zip_zlibraw_spak_deflate(tb_zip_t* zip, tb_bstream_t* ist, tb_bstream_t* ost, tb_bool_t sync)
{
...
static tb_long_t tb_zip_zlibraw_spak_inflate(tb_zip_t* zip, tb_bstream_t* ist, tb_bstream_t* ost, tb_bool_t sync)
{
	tb_zip_zlibraw_t* zlibraw = tb_zip_zlibraw_cast(zip);
	tb_assert_and_check_return_val(zlibraw && ist && ost, -1);
...
{	
	// alloc
	tb_zip_t* zip = (tb_zip_t*)tb_nalloc0(1, sizeof(tb_zip_zlibraw_t));
